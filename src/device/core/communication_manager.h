...
/**
 * @brief 通信管理�?- 统一管理WebSocket连接和消息处�? * 
 * 该类提供了设备与服务器之间的通信抽象层，支持�? * - 自动重连机制
 * - 消息队列管理
 * - 连接状态监�? * - 线程安全的消息发�? */
...
public:
    ...
    /**
     * @brief 发送消�?     * @param message 要发送的消息
     * @return 发送是否成�?     */
    ...
    /**
     * @brief 设置消息处理�?     * @param handler 消息处理回调函数
     */
    void setMessageHandler(MessageHandler handler);

    /**
     * @brief 设置连接状态处理器
     * @param handler 连接状态变化回调函�?     */
    void setConnectionStateHandler(ConnectionStateHandler handler);

    /**
     * @brief 获取当前连接状�?     * @return 当前连接状�?     */
    ConnectionState getConnectionState() const;

    /**
     * @brief 是否已连�?     * @return 连接状�?     */
    bool isConnected() const;

    /**
     * @brief 启用自动重连
     * @param enable 是否启用
     * @param retryInterval 重连间隔（秒�?     * @param maxRetries 最大重连次数（0表示无限重连�?     */
...
private:
    ...
    /**
     * @brief 更新连接状�?     * @param newState 新状�?     * @param error 错误信息（可选）
     */
...
private:
    ...
    // 连接状�?    mutable std::mutex stateMutex_;
