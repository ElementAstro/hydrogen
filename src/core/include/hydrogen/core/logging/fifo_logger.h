...
struct FifoMetrics {
  ...
  // Copy constructor - explicitly load atomic values
  FifoMetrics(const FifoMetrics& other)
      : totalMessages(other.totalMessages.load()),
        ...
        totalDisconnections(other.totalDisconnections.load()),
        totalReconnections(other.totalReconnections.load()),
        totalLatencyMicros(other.totalLatencyMicros.load()),
        maxLatencyMicros(other.maxLatencyMicros.load()),
        minLatencyMicros(other.minLatencyMicros.load()),
        startTime(other.startTime),
        lastResetTime(other.lastResetTime) {}

  // Copy assignment operator
  FifoMetrics& operator=(const FifoMetrics& other) {
    if (this != &other) {
      ...
      totalDisconnections.store(other.totalDisconnections.load());
      totalReconnections.store(other.totalReconnections.load());
      totalLatencyMicros.store(other.totalLatencyMicros.load());
      maxLatencyMicros.store(other.maxLatencyMicros.load());
      minLatencyMicros.store(other.minLatencyMicros.load());
      startTime = other.startTime;
      lastResetTime = other.lastResetTime;
    }
    return *this;
...
public:
  ...
  void log(FifoLogLevel level, const std::string &category,
  ...
  // Message tracing
  void traceMessage(
      const std::string &messageId, const std::string &clientId,
  ...
  std::vector<FifoMessageTrace> getMessageTraces(
      const std::string &clientId = "",
      std::chrono::milliseconds maxAge = std::chrono::milliseconds(0)) const;
  void clearMessageTraces();

  // Performance metrics
  void recordMessage(size_t bytes, std::chrono::microseconds latency =
...
public:
  ...
  identifyPerformanceIssues(const FifoMetrics &metrics);

  // Connection analysis
  static json analyzeConnections(const std::vector<FifoMessageTrace> &traces);
  static std::string
  generateConnectionReport(const std::vector<FifoMessageTrace> &traces);
  ...
  static std::vector<FifoLogEntry>
  filterLogs(const std::vector<FifoLogEntry> &logs,
...
#define FIFO_TRACE_MESSAGE(messageId, clientId, pipePath, direction, size,     \
                           type, content, time)                                \
  getGlobalFifoLogger().traceMessage(messageId, clientId, pipePath, direction, \
                                     size, type, content, time)
