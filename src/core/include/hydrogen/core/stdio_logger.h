...
public:
     ...
     */
    struct LoggerConfig {
        StdioLogLevel logLevel = StdioLogLevel::INFO;
        bool enableConsoleLogging = true;
        bool enableFileLogging = true;
        std::string logFileName = "stdio_communication.log";
        size_t maxFileSize = 10 * 1024 * 1024; // 10MB
        size_t maxFiles = 5;
        bool enableMessageTracing = false;
        bool enablePerformanceMetrics = true;
        bool enableDebugMode = false;
        std::string logPattern = "[%Y-%m-%d %H:%M:%S.%e] [%^%l%$] [%n] %v";
    ...
    void traceIncomingMessage(const std::string& messageId, const std::string& clientId, 
                             const std::string& messageType, const json& content, size_t size);
    void traceOutgoingMessage(const std::string& messageId, const std::string& clientId, 
                             const std::string& messageType, const json& content, size_t size);
    ...
    // Connection logging
    void logClientConnect(const std::string& clientId, const std::unordered_map<std::string, std::string>& metadata = {});
    void logClientDisconnect(const std::string& clientId, const std::string& reason = "");
    void logConnectionError(const std::string& clientId, const std::string& error);

    // Protocol logging
    void logProtocolEvent(const std::string& event, const std::string& details, const std::string& clientId = "");
    void logProtocolError(const std::string& error, const std::string& clientId = "");
    void logValidationError(const std::string& messageId, const std::string& error, const std::string& clientId = "");

    // Performance metrics
    void recordMessage(bool success, size_t bytes, std::chrono::microseconds processingTime);
    ...
    void setDebugFilter(const std::vector<std::string>& clientIds, const std::vector<std::string>& messageTypes);
    ...
    std::vector<MessageTrace> getMessageHistory(const std::string& clientId = "") const;
...
private:
    ...
    bool shouldTrace(const std::string& clientId, const std::string& messageType) const;
    void addToHistory(const MessageTrace& trace);
    std::string formatLogMessage(const std::string& message, const std::string& clientId) const;
 ...
 */
#define STDIO_LOG_TRACE(msg, clientId) getGlobalStdioLogger().trace(msg, clientId)
#define STDIO_LOG_DEBUG(msg, clientId) getGlobalStdioLogger().debug(msg, clientId)
...
#define STDIO_LOG_ERROR(msg, clientId) getGlobalStdioLogger().error(msg, clientId)
#define STDIO_LOG_CRITICAL(msg, clientId) getGlobalStdioLogger().critical(msg, clientId)

#define STDIO_TRACE_MESSAGE(tracer) MessageTracer tracer(getGlobalStdioLogger(), __VA_ARGS__)
