...
protected:
    ...
    bool validateMessageId(const std::string& messageId, ValidationResult& result) const;
    bool validateDeviceId(const std::string& deviceId, ValidationResult& result) const;
    bool validateTimestamp(const std::string& timestamp, ValidationResult& result) const;
    bool validateMessageType(MessageType type, ValidationResult& result) const;
    bool validatePriority(Message::Priority priority, ValidationResult& result) const;
    bool validateQoSLevel(Message::QoSLevel qos, ValidationResult& result) const;
    
    // Security validation methods
    bool checkForMaliciousContent(const std::string& content, ValidationResult& result) const;
    bool checkForSqlInjection(const std::string& content, ValidationResult& result) const;
    bool checkForXssAttempts(const std::string& content, ValidationResult& result) const;
    bool checkForPathTraversal(const std::string& content, ValidationResult& result) const;
    bool validateJsonStructure(const json& data, size_t currentDepth, ValidationResult& result) const;
...
public:
    explicit CommandMessageValidator(const SecurityRules& rules = SecurityRules{});
    
    ValidationResult validate(const Message& message) const override;
    ValidationResult validateJson(const json& messageJson) const override;

private:
    bool validateCommand(const std::string& command, ValidationResult& result) const;
    bool validateParameters(const json& parameters, ValidationResult& result) const;
    bool validateProperties(const json& properties, ValidationResult& result) const;
...
public:
    explicit ResponseMessageValidator(const SecurityRules& rules = SecurityRules{});
    
    ValidationResult validate(const Message& message) const override;
    ValidationResult validateJson(const json& messageJson) const override;

private:
    bool validateStatus(const std::string& status, ValidationResult& result) const;
...
private:
    bool validateEventName(const std::string& eventName, ValidationResult& result) const;
...
private:
    ...
    bool validateErrorMessage(const std::string& errorMessage, ValidationResult& result) const;
};
...
public:
    static std::unique_ptr<MessageValidatorInterface> createValidator(MessageType type, const SecurityRules& rules = SecurityRules{});
    static std::unique_ptr<BaseMessageValidator> createBaseValidator(const SecurityRules& rules = SecurityRules{});
    static std::unique_ptr<CommandMessageValidator> createCommandValidator(const SecurityRules& rules = SecurityRules{});
    static std::unique_ptr<ResponseMessageValidator> createResponseValidator(const SecurityRules& rules = SecurityRules{});
    static std::unique_ptr<EventMessageValidator> createEventValidator(const SecurityRules& rules = SecurityRules{});
    static std::unique_ptr<ErrorMessageValidator> createErrorValidator(const SecurityRules& rules = SecurityRules{});
};
