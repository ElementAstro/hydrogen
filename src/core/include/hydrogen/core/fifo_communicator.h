...
struct FifoStatistics {
    ...
    FifoStatistics() : startTime(std::chrono::system_clock::now()), 
...
private:
    ...
    // Connection state
    std::atomic<FifoConnectionState> connectionState_{FifoConnectionState::DISCONNECTED};
...
public:
    ...
    static std::unique_ptr<FifoCommunicator> createWithPreset(FifoConfigManager::ConfigPreset preset);
    
    // Platform-specific factories
    static std::unique_ptr<FifoCommunicator> createForWindows(const FifoConfig& config);
    static std::unique_ptr<FifoCommunicator> createForUnix(const FifoConfig& config);
    
    // Specialized factories
    static std::unique_ptr<FifoCommunicator> createBidirectional(const FifoConfig& config);
    static std::unique_ptr<FifoCommunicator> createHighPerformance(const FifoConfig& config);
    static std::unique_ptr<FifoCommunicator> createReliable(const FifoConfig& config);
};
...
public:
    ...
    static std::string generateUniquePipeName(const std::string& baseName = "hydrogen_fifo");
