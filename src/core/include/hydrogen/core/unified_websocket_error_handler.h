...
public:
  ...
  std::chrono::system_clock::time_point getLastFailureTime() const {
...
/**
 * @brief Unified WebSocket error handler that standardizes error handling
...
public:
  ...
  using RecoveryStrategyCallback =
      std::function<WebSocketRecoveryAction(const EnhancedWebSocketError &)>;
  using ConnectionRecoveryCallback = std::function<bool(
  ...
  void correlateError(const std::string &correlationId,
                      const EnhancedWebSocketError &error);
  std::vector<EnhancedWebSocketError>
  getCorrelatedErrors(const std::string &correlationId) const;

  // Circuit breaker management
  std::shared_ptr<WebSocketCircuitBreaker>
  getCircuitBreaker(const std::string &connectionId);
  ...
  void setErrorEventCallback(ErrorEventCallback callback) {
    errorEventCallback_ = callback;
  }
  void setRecoveryStrategyCallback(RecoveryStrategyCallback callback) {
    recoveryStrategyCallback_ = callback;
  }
  void setConnectionRecoveryCallback(ConnectionRecoveryCallback callback) {
    connectionRecoveryCallback_ = callback;
  }

  // Configuration
  void setGlobalRetryPolicy(int maxAttempts,
                            std::chrono::milliseconds baseDelay,
                            bool exponentialBackoff = true);
  void setConnectionSpecificRetryPolicy(const std::string &connectionId,
                                        int maxAttempts,
                                        std::chrono::milliseconds baseDelay);
  void enableCircuitBreaker(bool enable) { circuitBreakerEnabled_ = enable; }
  void setErrorCorrelationWindow(std::chrono::milliseconds window) {
    correlationWindow_ = window;
  }
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<WebSocketCircuitBreaker>>
      circuitBreakers_;

  // Error correlation
  mutable std::mutex correlationMutex_;
  std::unordered_map<std::string, std::vector<EnhancedWebSocketError>>
      correlatedErrors_;
  ...
  void updateStatistics(const EnhancedWebSocketError &error,
                        WebSocketRecoveryAction action,
  ...
  bool executeRecoveryAction(const std::string &connectionId,
                             WebSocketRecoveryAction action);
...
public:
  ...
  static std::shared_ptr<UnifiedWebSocketErrorHandler>
  createHandlerWithDefaults();
...
public:
  ...
  getComponentHandler(const std::string &component);
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<UnifiedWebSocketErrorHandler>>
      componentHandlers_;
};
...
#define HANDLE_ENHANCED_WEBSOCKET_ERROR(error, component)                      \
  UnifiedWebSocketErrorRegistry::getInstance().handleEnhancedError(error,      \
                                                                   component);

#define CREATE_WEBSOCKET_CONNECTION_CONTEXT(id, component, endpoint, isClient) \
  WebSocketConnectionContext {                                                 \
    id, component, endpoint, isClient, std::chrono::system_clock::now(),       \
