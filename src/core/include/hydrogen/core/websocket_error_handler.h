...
public:
    ...
    virtual WebSocketRecoveryAction determineRecoveryAction(const WebSocketError& error) = 0;
    virtual bool shouldRetry(const WebSocketError& error, int attemptCount) = 0;
    virtual std::chrono::milliseconds getRetryDelay(const WebSocketError& error, int attemptCount) = 0;
};
...
public:
    ...
    using RecoveryCallback = std::function<bool(const WebSocketError&, WebSocketRecoveryAction)>;
    ...
    void setBaseRetryDelay(std::chrono::milliseconds delay) { baseRetryDelay_ = delay; }
    void setMaxRetryDelay(std::chrono::milliseconds delay) { maxRetryDelay_ = delay; }
    void setUseExponentialBackoff(bool enable) { useExponentialBackoff_ = enable; }
...
private:
    ...
    WebSocketRecoveryAction getDefaultRecoveryAction(WebSocketErrorCategory category, WebSocketErrorSeverity severity);
};
...
public:
    ...
    static WebSocketError createFromBoostError(const boost::system::error_code& ec, 
    ...
    static WebSocketError createConnectionError(const std::string& message, const std::string& details = "");
    static WebSocketError createProtocolError(const std::string& message, const std::string& details = "");
    static WebSocketError createTimeoutError(const std::string& operation, std::chrono::milliseconds timeout);
    static WebSocketError createMessageError(const std::string& message, const std::string& details = "");
    static WebSocketError createAuthenticationError(const std::string& message, const std::string& details = "");
    
    // Utility methods
    static WebSocketErrorCategory categorizeBoostError(const boost::system::error_code& ec);
    static WebSocketErrorSeverity determineSeverity(WebSocketErrorCategory category, const boost::system::error_code& ec);
...
public:
    ...
    WebSocketWrapper(std::unique_ptr<WebSocketType> ws, ErrorHandler errorHandler);
    ~WebSocketWrapper();
    
    // Connection management
    bool connect(const std::string& host, const std::string& port, const std::string& target = "/ws");
    void disconnect();
    ...
    // Configuration
    void setReadTimeout(std::chrono::milliseconds timeout) { readTimeout_ = timeout; }
    void setWriteTimeout(std::chrono::milliseconds timeout) { writeTimeout_ = timeout; }
    void setAutoReconnect(bool enable) { autoReconnect_ = enable; }
    void setMaxReconnectAttempts(int attempts) { maxReconnectAttempts_ = attempts; }
...
private:
    ...
    void handleConnectionError(const boost::system::error_code& ec, const std::string& operation);
...
public:
    ...
    // Handler management
    void registerHandler(const std::string& component, std::shared_ptr<WebSocketErrorHandler> handler);
    void unregisterHandler(const std::string& component);
    std::shared_ptr<WebSocketErrorHandler> getHandler(const std::string& component);
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<WebSocketErrorHandler>> handlers_;
