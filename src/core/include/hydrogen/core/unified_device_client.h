 ...
 * 
 * This class consolidates the functionality of both DeviceClient and DeviceClientRefactored
 * into a single, robust implementation that supports multiple protocols and provides
 * comprehensive error handling, reconnection, and message processing capabilities.
 */
...
public:
    ...
    using PropertyChangeCallback = std::function<void(const std::string& deviceId, const std::string& property, const json& value)>;
    using EventCallback = std::function<void(const std::string& deviceId, const std::string& eventType, const json& data)>;
    using ErrorCallback = std::function<void(const std::string& error)>;

    explicit UnifiedDeviceClient(const ClientConnectionConfig& config = ClientConnectionConfig{});
    ~UnifiedDeviceClient();
    ...
    // Auto-reconnection
    void setAutoReconnect(bool enable, std::chrono::milliseconds interval = std::chrono::milliseconds{5000}, int maxAttempts = 0);
    ...
    // Message Operations
    json sendMessage(std::shared_ptr<Message> message, std::chrono::milliseconds timeout = std::chrono::milliseconds{5000});
    void sendMessageAsync(std::shared_ptr<Message> message, std::function<void(const json&)> callback = nullptr);
    
    // Subscription Management
    bool subscribeToProperty(const std::string& deviceId, const std::string& property);
    bool unsubscribeFromProperty(const std::string& deviceId, const std::string& property);
    bool subscribeToEvents(const std::string& deviceId, const std::string& eventType = "");
    bool unsubscribeFromEvents(const std::string& deviceId, const std::string& eventType = "");
    ...
    void setPropertyChangeCallback(PropertyChangeCallback callback) { propertyChangeCallback_ = callback; }
    ...
    // Error Handling
    void setErrorHandler(std::shared_ptr<WebSocketErrorHandler> handler) { errorHandler_ = handler; }
...
private:
    ...
    std::unordered_map<std::string, std::function<void(const json&)>> asyncCallbacks_;
    ...
    std::unordered_map<std::string, std::vector<std::string>> propertySubscriptions_;
...
public:
    static std::unique_ptr<UnifiedDeviceClient> createClient(const ClientConnectionConfig& config = ClientConnectionConfig{});
    static std::unique_ptr<UnifiedDeviceClient> createAndConnect(const std::string& host, uint16_t port);
    
    // Protocol-specific clients
    static std::unique_ptr<UnifiedDeviceClient> createWebSocketClient(const std::string& host, uint16_t port);
    static std::unique_ptr<UnifiedDeviceClient> createHttpClient(const std::string& host, uint16_t port);
    
    // Configuration presets
    static ClientConnectionConfig getDefaultConfig();
    static ClientConnectionConfig getSecureConfig(const std::string& host, uint16_t port);
    static ClientConnectionConfig getHighPerformanceConfig(const std::string& host, uint16_t port);
};
