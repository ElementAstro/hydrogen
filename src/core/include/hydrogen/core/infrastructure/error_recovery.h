...
#include "../device/device_health.h"
#include "../device/device_lifecycle.h"
...
public:
  ...
  void registerErrorHandlerWithSeverity(
      const std::string &errorCode, ErrorSeverity severity,
      ErrorHandlingStrategy strategy, ErrorHandlerFunc customHandler = nullptr);
  ...
  bool attemptRecovery(const std::string &deviceId,
                       const ErrorContext &context);
  bool escalateError(const ErrorContext &context);

  // Recovery sequence management
  void
  registerRecoverySequence(const std::string &deviceType,
                           const std::vector<ErrorHandlingStrategy> &sequence);
  bool executeRecoverySequence(const std::string &deviceId,
                               const ErrorContext &context);
...
private:
  ...
  std::map<std::string, std::map<std::string, ErrorHandlerInfo>>
      deviceHandlers_;
  ...
  // Internal methods
  bool executeErrorHandler(const ErrorContext &context,
