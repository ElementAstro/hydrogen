...
#include "../device/device_plugin.h"
#include <atomic>
#include <chrono>
#include <functional>
#include <memory>
...
public:
   ...
   */
  virtual bool
  startDiscovery(const DiscoveryFilter &filter = DiscoveryFilter{}) = 0;
   ...
   */
  virtual void registerConfigurationTemplate(
      const ConfigurationTemplate &configTemplate) = 0;
   ...
   */
  virtual std::optional<ConfigurationTemplate>
  getConfigurationTemplate(const std::string &deviceType,
   ...
   */
  virtual void registerDiscoveryHandler(
   ...
   */
  virtual void setDeviceFoundCallback(
      std::function<void(const DiscoveredDevice &)> callback) = 0;
   ...
   */
  virtual void
  setDeviceLostCallback(std::function<void(const std::string &)> callback) = 0;
...
public:
  ...
  bool
  startDiscovery(const DiscoveryFilter &filter = DiscoveryFilter{}) override;
  ...
  void registerConfigurationTemplate(
      const ConfigurationTemplate &configTemplate) override;
  std::optional<ConfigurationTemplate>
  getConfigurationTemplate(const std::string &deviceType,
                           const std::string &manufacturer,
                           const std::string &model) const override;
  void registerDiscoveryHandler(
      DiscoveryMethod method,
      std::function<std::vector<DiscoveredDevice>()> handler) override;
  void setDeviceFoundCallback(
      std::function<void(const DiscoveredDevice &)> callback) override;
...
private:
  ...
  bool matchesFilter(const DiscoveredDevice &device,
  ...
  std::unordered_map<DiscoveryMethod,
                     std::function<std::vector<DiscoveredDevice>()>>
      discoveryHandlers_;
