...
public:
    ...
    // Message operations
    bool sendMessage(const std::string& content, ZmqMessageType type = ZmqMessageType::DATA);
    bool sendMessage(const ZmqMessage& message);
    bool sendMessageSync(const std::string& content, std::string& response, int timeoutMs = 5000);
...
public:
    static std::unique_ptr<ZmqQueueClient> createClient(const ZmqClientConfig& config);
    static std::unique_ptr<ZmqQueueClient> createReqClient(const std::string& serverAddress);
    static std::unique_ptr<ZmqQueueClient> createSubClient(const std::string& serverAddress, const std::string& topic = "");
    static std::unique_ptr<ZmqQueueClient> createPullClient(const std::string& serverAddress);
...
public:
    ...
    // Client management
    void registerClient(const std::string& clientId, std::shared_ptr<ZmqQueueClient> client);
    ...
    std::vector<std::pair<std::string, ZmqClientStatistics>> getAllClientStatistics() const;
