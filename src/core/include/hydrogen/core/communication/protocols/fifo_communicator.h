...
#include "../../configuration/fifo_config_manager.h"
#include <atomic>
#include <chrono>
...
#ifdef _WIN32
#include <windows.h>
#include <sys/types.h>  // For mode_t on Windows
#else
#include <fcntl.h>
#include <sys/stat.h>
#include <unistd.h>
...
struct FifoStatistics {
  ...
  FifoStatistics(const FifoStatistics& other)
      : messagesSent(other.messagesSent.load()),
        ...
        connectionAttempts(other.connectionAttempts.load()),
        reconnectionAttempts(other.reconnectionAttempts.load()),
        errors(other.errors.load()),
        startTime(other.startTime),
        lastActivity(other.lastActivity) {}

  // Copy assignment operator
  FifoStatistics& operator=(const FifoStatistics& other) {
    if (this != &other) {
      ...
      connectionAttempts.store(other.connectionAttempts.load());
      reconnectionAttempts.store(other.reconnectionAttempts.load());
...
public:
  ...
  virtual void stop() {} // Default implementation to avoid pure virtual call in destructor
