...
#include "../../messaging/message_transformer.h"
#include <memory>
#include <mutex>
...
public:
  ...
  convertFromProtocol(const json &protocolData) const = 0;
...
public:
  ...
  convertFromProtocol(const json &protocolData) const override;
...
public:
  enum class SocketPattern { REQ_REP, PUB_SUB, PUSH_PULL };
...
public:
  static std::unique_ptr<ProtocolConverter>
  createConverter(MessageFormat format);
  ...
  static std::unique_ptr<ProtocolConverter>
  createZeroMqConverter(ZeroMqConverter::SocketPattern pattern =
                            ZeroMqConverter::SocketPattern::REQ_REP);
  static std::unique_ptr<ProtocolConverter> createGrpcProtobufConverter();
  static std::unique_ptr<ProtocolConverter>
  createCommunicationServiceConverter();
...
public:
  ...
  void registerConverter(MessageFormat format,
...
private:
  ...
  std::unordered_map<MessageFormat, std::unique_ptr<ProtocolConverter>>
      converters_;
