...
#include "../../device/device_communicator.h"
#include <atomic>
...
public:
  ...
  virtual bool publish(const std::string &topic, const std::string &message,
                       int qos = 1) = 0;
  virtual bool publish(const std::string &topic, const json &message,
                       int qos = 1) = 0;
...
public:
  ...
  sendUnaryRequest(const CommunicationMessage &message) = 0;

  // Streaming RPC
  virtual bool startClientStreaming(const std::string &method) = 0;
  virtual bool startServerStreaming(const std::string &method,
                                    const CommunicationMessage &request) = 0;
...
public:
  ...
  using MessageHandler =
      std::function<void(const std::vector<std::string> &multipart)>;
  using ErrorHandler = std::function<void(const std::string &error)>;
  ...
  virtual bool send(const std::vector<std::string> &multipart,
  ...
  virtual bool receive(std::vector<std::string> &multipart,
                       bool nonBlocking = false) = 0;
...
/**
 * @brief Abstract base class for TCP protocol communication
 */
class TcpProtocolCommunicator {
public:
  using MessageHandler = std::function<void(const std::string &message,
                                            const std::string &clientId)>;
  using ConnectionHandler =
      std::function<void(bool connected, const std::string &clientId)>;
  using ErrorHandler = std::function<void(const std::string &error)>;

  TcpProtocolCommunicator(const TcpConfig &config);
  virtual ~TcpProtocolCommunicator();
  ...
  virtual bool sendMessage(const json &message,
                           const std::string &clientId = "") = 0;
  virtual bool
  broadcastMessage(const std::string &message) = 0; // Server mode only
...
public:
  static std::unique_ptr<MqttCommunicator>
  createMqttCommunicator(const MqttConfig &config);
  static std::unique_ptr<GrpcCommunicator>
  createGrpcCommunicator(const GrpcConfig &config);
  static std::unique_ptr<ZmqCommunicator>
  createZmqCommunicator(const ZmqConfig &config,
                        ZmqCommunicator::SocketType socketType);
  static std::unique_ptr<TcpCommunicator>
  createTcpCommunicator(const TcpConfig &config);
  static std::unique_ptr<StdioCommunicator>
  createStdioCommunicator(const StdioConfig &config);
};
...
public:
  ...
  // Communication
  bool sendMessage(const CommunicationMessage &message,
                   CommunicationProtocol protocol);
  bool broadcastMessage(const CommunicationMessage &message);

  // Event handling
  using MessageHandler = std::function<void(const CommunicationMessage &message,
                                            CommunicationProtocol protocol)>;
  using ConnectionHandler =
      std::function<void(CommunicationProtocol protocol, bool connected)>;
...
private:
  ...
  std::unordered_map<CommunicationProtocol, std::unique_ptr<MqttCommunicator>>
      mqttCommunicators_;
  std::unordered_map<CommunicationProtocol, std::unique_ptr<GrpcCommunicator>>
      grpcCommunicators_;
  std::unordered_map<CommunicationProtocol, std::unique_ptr<ZmqCommunicator>>
      zmqCommunicators_;
  std::unordered_map<CommunicationProtocol, std::unique_ptr<TcpCommunicator>>
      tcpCommunicators_;
  std::unordered_map<CommunicationProtocol, std::unique_ptr<StdioCommunicator>>
      stdioCommunicators_;
