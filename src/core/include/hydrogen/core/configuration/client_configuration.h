...
public:
  ...
  void registerConfiguration(const std::string &name,
                             std::shared_ptr<ClientConfiguration> config);
  ...
  validateAllConfigurations() const;

  // Configuration watching
  using ConfigChangeCallback =
      std::function<void(const std::string &, const ClientConfiguration &)>;
  void setConfigChangeCallback(ConfigChangeCallback callback);
  void watchConfigurationFile(const std::string &name,
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<ClientConfiguration>>
      configurations_;
...
public:
  ...
  // Features
  ConfigurationBuilder &withFeature(const std::string &feature,
                                    bool enable = true);
