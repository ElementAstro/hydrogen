...
/**
 * @brief Protocol error mapper - converts exceptions to protocol-specific errors
 */
...
public:
    ...
    // Register formatters
    void registerFormatter(MessageFormat format, std::unique_ptr<ProtocolErrorFormatter> formatter);
    
    // Register exception handlers
    void registerExceptionHandler(const std::string& exceptionType, ExceptionHandler handler);
    
    // Error mapping
    ProtocolError mapException(const std::exception& ex, const std::string& component = "", const std::string& operation = "") const;
    ProtocolError createError(ProtocolErrorCode code, const std::string& message, const std::string& details = "") const;
    
    // Protocol-specific formatting
    json formatErrorForProtocol(const ProtocolError& error, MessageFormat format) const;
    std::unique_ptr<ErrorMessage> createErrorMessage(const ProtocolError& error, const std::string& originalMessageId = "") const;
...
private:
    std::unordered_map<MessageFormat, std::unique_ptr<ProtocolErrorFormatter>> formatters_;
...
#define HANDLE_PROTOCOL_ERROR(ex, component, operation, format) \
    do { \
        auto error = getGlobalProtocolErrorMapper().mapException(ex, component, operation); \
        auto formattedError = getGlobalProtocolErrorMapper().formatErrorForProtocol(error, format); \
...
#define CREATE_PROTOCOL_ERROR_MESSAGE(code, message, details, originalId) \
    getGlobalProtocolErrorMapper().createErrorMessage( \
        getGlobalProtocolErrorMapper().createError(code, message, details), originalId)
