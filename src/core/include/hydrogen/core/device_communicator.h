...
struct StdioConfig {
  ...
  enum class CompressionType { NONE, GZIP, ZLIB, LZ4 };
...
public:
   ...
   */
  virtual std::future<CommunicationResponse>
  sendMessage(const CommunicationMessage &message) = 0;
  ...
  sendMessageSync(const CommunicationMessage &message) = 0;
  ...
  virtual void setMessageCallback(
      std::function<void(const CommunicationMessage &)> callback) = 0;
   ...
   */
  virtual void
  setConnectionStatusCallback(std::function<void(bool)> callback) = 0;
   ...
   */
  virtual void setEncryptionEnabled(bool enabled,
...
public:
   ...
   */
  static std::unique_ptr<IDeviceCommunicator>
  createCommunicator(CommunicationProtocol protocol);
   ...
   */
  static void registerCommunicatorFactory(
      CommunicationProtocol protocol,
...
private:
  static std::unordered_map<
      ...
      std::function<std::unique_ptr<IDeviceCommunicator>()>>
      factories_;
...
public:
  ...
  std::future<CommunicationResponse>
  sendMessage(const CommunicationMessage &message) override;
  CommunicationResponse
  sendMessageSync(const CommunicationMessage &message) override;
  void setMessageCallback(
      std::function<void(const CommunicationMessage &)> callback) override;
...
CommunicationProtocol
stringToCommunicationProtocol(const std::string &protocol);
