...
public:
     ...
     */
    virtual bool initialize(const PluginSecurityContext& context, const json& config) = 0;
     ...
     */
    virtual bool validateDeviceConfig(const std::string& deviceType, const json& config) const = 0;
     ...
     */
    virtual json handleCommand(const std::string& command, const json& parameters) = 0;
...
public:
     ...
     */
    std::vector<std::string> getSupportedDeviceTypes(const std::string& pluginId) const;
     ...
     */
    void setPluginEventCallback(std::function<void(const std::string&, const std::string&, const json&)> callback);
...
private:
    ...
    // Event callback
    std::function<void(const std::string&, const std::string&, const json&)> eventCallback_;
    
    // Helper methods
    PluginSecurityContext createSecurityContext(const PluginMetadata& metadata) const;
    bool validateSecurity(const PluginMetadata& metadata, const PluginSecurityContext& context) const;
    ...
    void notifyPluginEvent(const std::string& pluginId, const std::string& event, const json& data);
};
