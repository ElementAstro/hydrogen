...
public:
    ...
    virtual void registerDevice(const std::string& deviceId, 
                               DeviceLifecycleState initialState = DeviceLifecycleState::UNINITIALIZED) = 0;
     ...
     */
    virtual DeviceLifecycleState getCurrentState(const std::string& deviceId) const = 0;
     ...
     */
    virtual std::vector<DeviceLifecycleState> getValidNextStates(const std::string& deviceId) const = 0;
     ...
     */
    virtual std::vector<StateTransition> getStateHistory(const std::string& deviceId, 
                                                        size_t maxEntries = 0) const = 0;
     ...
     */
    virtual void forceErrorState(const std::string& deviceId, const std::string& errorReason) = 0;
     ...
     */
    virtual std::vector<std::string> getDevicesInState(DeviceLifecycleState state) const = 0;
...
public:
    ...
    void registerDevice(const std::string& deviceId, 
                       DeviceLifecycleState initialState = DeviceLifecycleState::UNINITIALIZED) override;
    void unregisterDevice(const std::string& deviceId) override;
    bool transitionTo(const std::string& deviceId, 
                     DeviceLifecycleState newState,
                     const std::string& trigger,
                     const std::string& reason = "") override;
    DeviceLifecycleState getCurrentState(const std::string& deviceId) const override;
    bool isValidTransition(const std::string& deviceId,
                          DeviceLifecycleState fromState,
                          DeviceLifecycleState toState) const override;
    std::vector<DeviceLifecycleState> getValidNextStates(const std::string& deviceId) const override;
    std::vector<StateTransition> getStateHistory(const std::string& deviceId, 
                                                 size_t maxEntries = 0) const override;
    void setStateChangeCallback(std::function<void(const LifecycleEvent&)> callback) override;
    void forceErrorState(const std::string& deviceId, const std::string& errorReason) override;
    bool attemptRecovery(const std::string& deviceId) override;
    std::vector<std::string> getDevicesInState(DeviceLifecycleState state) const override;
...
private:
    ...
    std::unordered_map<DeviceLifecycleState, std::unordered_set<DeviceLifecycleState>> validTransitions_;
    
    // Helper methods
    void initializeValidTransitions();
    bool isTransitionAllowed(DeviceLifecycleState from, DeviceLifecycleState to) const;
    void addToHistory(const std::string& deviceId, const StateTransition& transition);
