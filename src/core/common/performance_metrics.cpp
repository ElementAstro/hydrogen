...
void PerformanceMetricsCollector::recordMeasurement(const std::string& deviceId, 
...
PerformanceStatistics PerformanceMetricsCollector::getStatistics(const std::string& deviceId,
                                                                MetricType metricType,
...
PerformanceMetricsCollector::getAllStatistics(const std::string& deviceId) const {
    std::lock_guard<std::mutex> lock(metricsMutex_);
...
PerformanceTrend PerformanceMetricsCollector::analyzeTrend(const std::string& deviceId,
                                                          MetricType metricType,
    ...
    if (deviceIt != deviceMetrics_.end()) {
        for (const auto& [metricKey, metricData] : deviceIt->second) {
            for (const auto& measurement : metricData.measurements) {
                if (measurement.type == metricType && measurement.timestamp >= cutoffTime) {
...
void PerformanceMetricsCollector::registerAlert(const std::string& deviceId, const PerformanceAlert& alert) {
    std::lock_guard<std::mutex> lock(alertsMutex_);
    deviceAlerts_[deviceId].push_back(alert);
}

void PerformanceMetricsCollector::removeAlert(const std::string& deviceId, const std::string& alertName) {
    std::lock_guard<std::mutex> lock(alertsMutex_);
...
void PerformanceMetricsCollector::setAlertCallback(
    std::function<void(const std::string&, const PerformanceAlert&, double)> callback) {
...
void PerformanceMetricsCollector::setAggregationInterval(std::chrono::seconds interval) {
...
std::string metricTypeToString(MetricType type) {
    switch (type) {
        case MetricType::RESPONSE_TIME: return "RESPONSE_TIME";
        case MetricType::THROUGHPUT: return "THROUGHPUT";
        case MetricType::ERROR_RATE: return "ERROR_RATE";
        case MetricType::CONNECTION_TIME: return "CONNECTION_TIME";
        case MetricType::MEMORY_USAGE: return "MEMORY_USAGE";
        case MetricType::CPU_USAGE: return "CPU_USAGE";
        case MetricType::NETWORK_LATENCY: return "NETWORK_LATENCY";
        case MetricType::QUEUE_DEPTH: return "QUEUE_DEPTH";
...
MetricType stringToMetricType(const std::string& type) {
    if (type == "RESPONSE_TIME") return MetricType::RESPONSE_TIME;
    if (type == "THROUGHPUT") return MetricType::THROUGHPUT;
    if (type == "ERROR_RATE") return MetricType::ERROR_RATE;
    if (type == "CONNECTION_TIME") return MetricType::CONNECTION_TIME;
    if (type == "MEMORY_USAGE") return MetricType::MEMORY_USAGE;
    if (type == "CPU_USAGE") return MetricType::CPU_USAGE;
    if (type == "NETWORK_LATENCY") return MetricType::NETWORK_LATENCY;
    if (type == "QUEUE_DEPTH") return MetricType::QUEUE_DEPTH;
    return MetricType::CUSTOM;
}

void PerformanceMetricsCollector::calculateStatistics(const std::string& deviceId, const std::string& metricKey) {
    auto& metricData = deviceMetrics_[deviceId][metricKey];
...
void PerformanceMetricsCollector::checkAlerts(const std::string& deviceId) {
    ...
    for (const auto& alert : alertIt->second) {
        ...
        if (alert.condition == "greater_than" && currentValue > alert.threshold) {
            ...
            alertTriggered = true;
        } else if (alert.condition == "equals" && std::abs(currentValue - alert.threshold) < 0.001) {
            alertTriggered = true;
        } else if (alert.condition == "not_equals" && std::abs(currentValue - alert.threshold) >= 0.001) {
            alertTriggered = true;
        }
...
void PerformanceMetricsCollector::startAggregationThread() {
    aggregationThread_ = std::thread(&PerformanceMetricsCollector::aggregationThreadFunction, this);
}
...
void PerformanceMetricsCollector::startAlertThread() {
    alertThread_ = std::thread(&PerformanceMetricsCollector::alertThreadFunction, this);
}
...
std::string PerformanceMetricsCollector::getMetricKey(MetricType type, const std::string& name) const {
    return metricTypeToString(type) + "::" + name;
}

void PerformanceMetricsCollector::trimMeasurements(std::vector<PerformanceMeasurement>& measurements) {
...
double PerformanceMetricsCollector::calculateMedian(std::vector<double> values) const {
...
double PerformanceMetricsCollector::calculateStandardDeviation(const std::vector<double>& values, double mean) const {
...
bool PerformanceMetricsCollector::exportMetrics(const std::string& filename, const std::string& format) const {
    try {
        std::lock_guard<std::mutex> lock(metricsMutex_);
