...
DeviceBase::DeviceBase(const std::string& deviceId, const std::string& deviceType,
...
DeviceBase::~DeviceBase() {
    // Note: Cannot call pure virtual functions stop() and disconnect() from destructor
    // Derived classes should handle cleanup in their own destructors
    running_ = false;
    connected_ = false;
}

std::string DeviceBase::getDeviceId() const {
    return deviceId_;
}

std::string DeviceBase::getDeviceType() const {
    return deviceType_;
}
...
void DeviceBase::registerCommandHandler(const std::string& command, CommandHandler handler) {
...
void DeviceBase::initializeProperties() {
    ...
    registerCommandHandler("get_properties", 
    ...
    registerCommandHandler("set_properties",
        [this](const CommandMessage& cmd, ResponseMessage& response) {
            auto params = cmd.getParameters();
...
void DeviceRegistry::registerDeviceType(const std::string& deviceType, DeviceFactory factory) {
    std::lock_guard<std::mutex> lock(factoriesMutex_);
    factories_[deviceType] = factory;
}

std::unique_ptr<IDevice> DeviceRegistry::createDevice(const std::string& deviceType,
