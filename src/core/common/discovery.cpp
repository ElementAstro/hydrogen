...
json DiscoveredDevice::toJson() const {
    ...
    return json{
        ...
        {"metadata", metadata},
        {"discoveryTime", std::chrono::duration_cast<std::chrono::milliseconds>(discoveryTime.time_since_epoch()).count()},
...
DiscoveredDevice DiscoveredDevice::fromJson(const json& j) {
    ...
    device.discoveryMethod = static_cast<DiscoveryMethod>(j.value("discoveryMethod", 0));
    ...
    if (j.contains("discoveryTime")) {
        ...
        device.discoveryTime = std::chrono::system_clock::time_point(std::chrono::milliseconds(ms));
    }
...
std::vector<DiscoveredDevice> DeviceDiscovery::getDiscoveredDevices(const DiscoveryFilter& filter) const {
...
void DeviceDiscovery::registerConfigurationTemplate(const ConfigurationTemplate& configTemplate) {
    ...
    SPDLOG_INFO("Registered configuration template for {}/{}/{}", 
               configTemplate.deviceType, configTemplate.manufacturer, configTemplate.model);
}

std::optional<ConfigurationTemplate> DeviceDiscovery::getConfigurationTemplate(
...
void DeviceDiscovery::registerDiscoveryHandler(DiscoveryMethod method,
    std::function<std::vector<DiscoveredDevice>()> handler) {
    
    std::lock_guard<std::mutex> lock(handlersMutex_);
    discoveryHandlers_[method] = handler;
    SPDLOG_INFO("Registered discovery handler for method {}", static_cast<int>(method));
}

void DeviceDiscovery::setDeviceFoundCallback(std::function<void(const DiscoveredDevice&)> callback) {
    deviceFoundCallback_ = callback;
}

void DeviceDiscovery::setDeviceLostCallback(std::function<void(const std::string&)> callback) {
...
json DeviceDiscovery::getDiscoveryStatistics() const {
    ...
    return json{
        ...
        {"devicesCurrentlyDiscovered", statistics_.devicesCurrentlyDiscovered.load()},
        {"discoveryScansPerformed", statistics_.discoveryScansPerformed.load()},
        {"autoConfigurationsAttempted", statistics_.autoConfigurationsAttempted.load()},
        {"autoConfigurationsSuccessful", statistics_.autoConfigurationsSuccessful.load()},
        {"lastDiscoveryTime", std::chrono::duration_cast<std::chrono::milliseconds>(statistics_.lastDiscoveryTime.time_since_epoch()).count()},
        {"discoveryStartTime", std::chrono::duration_cast<std::chrono::milliseconds>(statistics_.discoveryStartTime.time_since_epoch()).count()}
    };
}
...
void DeviceDiscovery::performDiscovery() {
    ...
    for (const auto& [method, handler] : discoveryHandlers_) {
        try {
            ...
            SPDLOG_ERROR("Discovery handler for method {} failed: {}", static_cast<int>(method), e.what());
        }
    }
...
void DeviceDiscovery::addDiscoveredDevice(const DiscoveredDevice& device) {
    ...
    bool isNew = discoveredDevices_.find(device.deviceId) == discoveredDevices_.end();
...
bool DeviceDiscovery::matchesFilter(const DiscoveredDevice& device, const DiscoveryFilter& filter) const {
    // Check device types
    if (!filter.deviceTypes.empty()) {
        if (std::find(filter.deviceTypes.begin(), filter.deviceTypes.end(), device.deviceType) == filter.deviceTypes.end()) {
            return false;
        }
    }
    ...
    if (!filter.manufacturers.empty()) {
        if (std::find(filter.manufacturers.begin(), filter.manufacturers.end(), device.manufacturer) == filter.manufacturers.end()) {
            return false;
        }
    }
    ...
    if (!filter.methods.empty()) {
        if (std::find(filter.methods.begin(), filter.methods.end(), device.discoveryMethod) == filter.methods.end()) {
            return false;
        }
    }
