...
void DeviceHealthMonitor::recordCommand(const std::string& deviceId, 
    ...
    if (metrics.totalCommands == 1) {
        ...
        metrics.averageResponseTime = 
            (metrics.averageResponseTime * (metrics.totalCommands - 1) + responseTimeMs) / metrics.totalCommands;
    ...
    if (success) {
        ...
        if (metrics.successfulConnections == 1) {
            ...
            metrics.averageConnectionTime = 
                (metrics.averageConnectionTime * (metrics.successfulConnections - 1) + connectionTimeMs) / 
                metrics.successfulConnections;
        }
    } else {
...
DeviceHealthStatus DeviceHealthMonitor::getHealthStatus(const std::string& deviceId) const {
    std::lock_guard<std::mutex> lock(metricsMutex_);
...
DeviceHealthReport DeviceHealthMonitor::getHealthReport(const std::string& deviceId) const {
    std::lock_guard<std::mutex> lock(metricsMutex_);
...
std::vector<DeviceHealthReport> DeviceHealthMonitor::getAllHealthReports() const {
    std::lock_guard<std::mutex> lock(metricsMutex_);
...
void DeviceHealthMonitor::setHealthThreshold(DeviceHealthStatus status, double threshold) {
    std::lock_guard<std::mutex> lock(metricsMutex_);
    healthThresholds_[status] = threshold;
}

void DeviceHealthMonitor::setHealthChangeCallback(
    std::function<void(const std::string&, DeviceHealthStatus, DeviceHealthStatus)> callback) {
...
std::string healthStatusToString(DeviceHealthStatus status) {
    switch (status) {
        case DeviceHealthStatus::EXCELLENT: return "EXCELLENT";
        case DeviceHealthStatus::GOOD: return "GOOD";
        case DeviceHealthStatus::FAIR: return "FAIR";
        case DeviceHealthStatus::POOR: return "POOR";
...
DeviceHealthStatus stringToHealthStatus(const std::string& status) {
    if (status == "EXCELLENT") return DeviceHealthStatus::EXCELLENT;
    if (status == "GOOD") return DeviceHealthStatus::GOOD;
    if (status == "FAIR") return DeviceHealthStatus::FAIR;
    if (status == "POOR") return DeviceHealthStatus::POOR;
    if (status == "CRITICAL") return DeviceHealthStatus::CRITICAL;
    return DeviceHealthStatus::UNKNOWN;
}

double DeviceHealthMonitor::calculateHealthScore(const DeviceMetrics& metrics) const {
    if (metrics.totalCommands == 0) {
    ...
    if (metrics.averageResponseTime > 0) {
        double responseTimePenalty = std::min(metrics.averageResponseTime / 1000.0, 1.0); // Normalize to 1 second
...
DeviceHealthStatus DeviceHealthMonitor::scoreToStatus(double score) const {
    if (score >= healthThresholds_.at(DeviceHealthStatus::EXCELLENT)) return DeviceHealthStatus::EXCELLENT;
    if (score >= healthThresholds_.at(DeviceHealthStatus::GOOD)) return DeviceHealthStatus::GOOD;
    if (score >= healthThresholds_.at(DeviceHealthStatus::FAIR)) return DeviceHealthStatus::FAIR;
    if (score >= healthThresholds_.at(DeviceHealthStatus::POOR)) return DeviceHealthStatus::POOR;
...
std::vector<std::string> DeviceHealthMonitor::generateHealthIssues(const DeviceMetrics& metrics) const {
    std::vector<std::string> issues;

    if (metrics.getSuccessRate() < 0.8) {
        issues.push_back("Low command success rate: " + std::to_string(metrics.getSuccessRate() * 100) + "%");
    }

    if (metrics.averageResponseTime > 1000.0) {
        issues.push_back("High average response time: " + std::to_string(metrics.averageResponseTime) + "ms");
    }

    if (metrics.getConnectionSuccessRate() < 0.9) {
        issues.push_back("Connection reliability issues: " + std::to_string(metrics.getConnectionSuccessRate() * 100) + "%");
    }

    if (metrics.criticalErrors > 0) {
        issues.push_back("Critical errors detected: " + std::to_string(metrics.criticalErrors));
    }

    if (metrics.memoryUsageMB > 100.0) {
        issues.push_back("High memory usage: " + std::to_string(metrics.memoryUsageMB) + "MB");
    }

    if (metrics.cpuUsagePercent > 80.0) {
        issues.push_back("High CPU usage: " + std::to_string(metrics.cpuUsagePercent) + "%");
    }

    return issues;
}

std::vector<std::string> DeviceHealthMonitor::generateRecommendations(const DeviceMetrics& metrics) const {
    std::vector<std::string> recommendations;

    if (metrics.getSuccessRate() < 0.8) {
        recommendations.push_back("Check device connection and configuration");
        recommendations.push_back("Review command parameters and device compatibility");
    }

    if (metrics.averageResponseTime > 1000.0) {
        recommendations.push_back("Consider optimizing device communication protocol");
        recommendations.push_back("Check network latency and bandwidth");
    }

    if (metrics.getConnectionSuccessRate() < 0.9) {
        recommendations.push_back("Verify network stability and device availability");
        recommendations.push_back("Consider implementing connection retry strategies");
    }
    ...
    if (metrics.memoryUsageMB > 100.0) {
        ...
        recommendations.push_back("Consider implementing memory optimization strategies");
    }
