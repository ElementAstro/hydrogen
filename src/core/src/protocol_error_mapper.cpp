...
std::string ProtocolError::toString() const {
    return "ProtocolError[" + std::to_string(static_cast<int>(code)) + "]: " + message + 
...
std::string HttpErrorFormatter::getErrorMessage(const ProtocolError& error) const {
...
std::string GrpcErrorFormatter::getErrorMessage(const ProtocolError& error) const {
    return error.message;
}
...
std::string MqttErrorFormatter::getErrorMessage(const ProtocolError& error) const {
    return error.message;
}
...
std::string ZeroMqErrorFormatter::getErrorMessage(const ProtocolError& error) const {
...
std::string AscomErrorFormatter::getErrorMessage(const ProtocolError& error) const {
    return error.message;
}
...
std::string AscomErrorFormatter::getAscomErrorSource(const ProtocolError& error) const {
...
std::string IndiErrorFormatter::getErrorMessage(const ProtocolError& error) const {
    return error.message;
}
...
void ProtocolErrorMapper::registerFormatter(MessageFormat format, std::unique_ptr<ProtocolErrorFormatter> formatter) {
    formatters_[format] = std::move(formatter);
    spdlog::debug("ProtocolErrorMapper: Registered formatter for format: {}", static_cast<int>(format));
}

void ProtocolErrorMapper::registerExceptionHandler(const std::string& exceptionType, ExceptionHandler handler) {
    exceptionHandlers_[exceptionType] = handler;
    spdlog::debug("ProtocolErrorMapper: Registered exception handler for type: {}", exceptionType);
}

ProtocolError ProtocolErrorMapper::mapException(const std::exception& ex, const std::string& component, const std::string& operation) const {
    ...
    if (it != exceptionHandlers_.end()) {
        try {
            ...
            spdlog::error("Exception handler failed for {}: {}", exceptionType, handlerEx.what());
        }
    }
...
ProtocolError ProtocolErrorMapper::createError(ProtocolErrorCode code, const std::string& message, const std::string& details) const {
    ProtocolError error;
    error.code = code;
...
json ProtocolErrorMapper::formatErrorForProtocol(const ProtocolError& error, MessageFormat format) const {
...
std::unique_ptr<ErrorMessage> ProtocolErrorMapper::createErrorMessage(const ProtocolError& error, const std::string& originalMessageId) const {
    auto errorMsg = std::make_unique<ErrorMessage>(std::to_string(static_cast<int>(error.code)), error.message);
...
void ProtocolErrorMapper::initializeDefaultFormatters() {
    registerFormatter(MessageFormat::HTTP_JSON, std::make_unique<HttpErrorFormatter>());
    registerFormatter(MessageFormat::PROTOBUF, std::make_unique<GrpcErrorFormatter>());
    registerFormatter(MessageFormat::MQTT, std::make_unique<MqttErrorFormatter>());
    registerFormatter(MessageFormat::ZEROMQ, std::make_unique<ZeroMqErrorFormatter>());

    // Note: ASCOM and INDI formatters would be registered when those protocols are enabled
...
void ProtocolErrorMapper::initializeDefaultExceptionHandlers() {
    ...
    registerExceptionHandler("std::runtime_error", [](const std::exception& ex) {
        return ErrorMappingUtils::mapRuntimeError(dynamic_cast<const std::runtime_error&>(ex));
    });

    registerExceptionHandler("std::logic_error", [](const std::exception& ex) {
        return ErrorMappingUtils::mapLogicError(dynamic_cast<const std::logic_error&>(ex));
    });

    registerExceptionHandler("std::invalid_argument", [](const std::exception& ex) {
        return ErrorMappingUtils::mapInvalidArgument(dynamic_cast<const std::invalid_argument&>(ex));
    });

    registerExceptionHandler("std::out_of_range", [](const std::exception& ex) {
        return ErrorMappingUtils::mapOutOfRange(dynamic_cast<const std::out_of_range&>(ex));
    });

    registerExceptionHandler("std::system_error", [](const std::exception& ex) {
        return ErrorMappingUtils::mapSystemError(dynamic_cast<const std::system_error&>(ex));
    });

    spdlog::info("ProtocolErrorMapper: Initialized default exception handlers");
}

ProtocolError ProtocolErrorMapper::mapStandardException(const std::exception& ex) const {
    return ErrorMappingUtils::mapStdException(ex);
}

std::string ProtocolErrorMapper::getExceptionTypeName(const std::exception& ex) const {
...
std::string ProtocolErrorMapper::getErrorCodeName(ProtocolErrorCode code) {
    switch (code) {
        case ProtocolErrorCode::SUCCESS: return "SUCCESS";
        case ProtocolErrorCode::UNKNOWN_ERROR: return "UNKNOWN_ERROR";
        case ProtocolErrorCode::INTERNAL_ERROR: return "INTERNAL_ERROR";
        case ProtocolErrorCode::INVALID_REQUEST: return "INVALID_REQUEST";
        case ProtocolErrorCode::INVALID_PARAMETERS: return "INVALID_PARAMETERS";
        case ProtocolErrorCode::OPERATION_FAILED: return "OPERATION_FAILED";
        case ProtocolErrorCode::CONNECTION_FAILED: return "CONNECTION_FAILED";
        case ProtocolErrorCode::CONNECTION_LOST: return "CONNECTION_LOST";
        case ProtocolErrorCode::CONNECTION_TIMEOUT: return "CONNECTION_TIMEOUT";
        case ProtocolErrorCode::AUTHENTICATION_FAILED: return "AUTHENTICATION_FAILED";
        case ProtocolErrorCode::AUTHORIZATION_FAILED: return "AUTHORIZATION_FAILED";
        case ProtocolErrorCode::PROTOCOL_ERROR: return "PROTOCOL_ERROR";
        case ProtocolErrorCode::UNSUPPORTED_OPERATION: return "UNSUPPORTED_OPERATION";
        case ProtocolErrorCode::MESSAGE_FORMAT_ERROR: return "MESSAGE_FORMAT_ERROR";
        case ProtocolErrorCode::PROTOCOL_VERSION_MISMATCH: return "PROTOCOL_VERSION_MISMATCH";
        case ProtocolErrorCode::DEVICE_NOT_FOUND: return "DEVICE_NOT_FOUND";
        case ProtocolErrorCode::DEVICE_BUSY: return "DEVICE_BUSY";
        case ProtocolErrorCode::DEVICE_ERROR: return "DEVICE_ERROR";
        case ProtocolErrorCode::DEVICE_DISCONNECTED: return "DEVICE_DISCONNECTED";
        case ProtocolErrorCode::DEVICE_TIMEOUT: return "DEVICE_TIMEOUT";
        case ProtocolErrorCode::RESOURCE_UNAVAILABLE: return "RESOURCE_UNAVAILABLE";
        case ProtocolErrorCode::RESOURCE_EXHAUSTED: return "RESOURCE_EXHAUSTED";
        case ProtocolErrorCode::QUOTA_EXCEEDED: return "QUOTA_EXCEEDED";
        case ProtocolErrorCode::VALIDATION_ERROR: return "VALIDATION_ERROR";
        case ProtocolErrorCode::MISSING_REQUIRED_FIELD: return "MISSING_REQUIRED_FIELD";
        case ProtocolErrorCode::INVALID_FIELD_VALUE: return "INVALID_FIELD_VALUE";
        case ProtocolErrorCode::FIELD_OUT_OF_RANGE: return "FIELD_OUT_OF_RANGE";
        default: return "UNKNOWN";
    }
}

ProtocolErrorCode ProtocolErrorMapper::parseErrorCode(const std::string& codeName) {
    static std::unordered_map<std::string, ProtocolErrorCode> codeMap = {
        ...
        {"PROTOCOL_VERSION_MISMATCH", ProtocolErrorCode::PROTOCOL_VERSION_MISMATCH},
...
ProtocolError ErrorMappingUtils::mapInvalidArgument(const std::invalid_argument& ex) {
    ProtocolError error;
...
ProtocolError ErrorMappingUtils::mapSystemError(const std::system_error& ex) {
    ...
    error.code = ProtocolErrorCode::INTERNAL_ERROR;
    error.message = ex.what();
    error.details = "System error: " + std::to_string(ex.code().value()) + " (" + ex.code().message() + ")";
    error.timestamp = std::chrono::system_clock::now();
    return error;
}
...
ProtocolError ErrorMappingUtils::mapConnectionException(const std::exception& ex) {
    ProtocolError error;
...
ProtocolError ErrorMappingUtils::mapProtocolException(const std::exception& ex) {
    ProtocolError error;
...
ProtocolError ErrorMappingUtils::mapValidationException(const std::exception& ex) {
    ProtocolError error;
