...
bool WebSocketError::isRecoverable() const {
  ...
  case WebSocketErrorCategory::MESSAGE:
    return severity == WebSocketErrorSeverity::LOW ||
...
WebSocketRecoveryAction StandardWebSocketErrorHandler::determineRecoveryAction(
...
bool StandardWebSocketErrorHandler::shouldRetry(const WebSocketError &error,
                                                int attemptCount) {
...
StandardWebSocketErrorHandler::getRetryDelay(const WebSocketError &error,
                                             int attemptCount) {
  ...
  delay = std::chrono::milliseconds(
      static_cast<long long>(delay.count() * dis(gen)));

  return delay;
}

StandardWebSocketErrorHandler::ErrorStatistics
StandardWebSocketErrorHandler::getStatistics() const {
  std::lock_guard<std::mutex> lock(statisticsMutex_);
  return statistics_;
}
...
void StandardWebSocketErrorHandler::updateStatistics(
    const WebSocketError &error) {
  std::lock_guard<std::mutex> lock(statisticsMutex_);
...
WebSocketRecoveryAction StandardWebSocketErrorHandler::getDefaultRecoveryAction(
    WebSocketErrorCategory category, WebSocketErrorSeverity severity) {
  switch (category) {
  case WebSocketErrorCategory::CONNECTION:
    return (severity >= WebSocketErrorSeverity::MEDIUM)
               ? WebSocketRecoveryAction::RECONNECT
               : WebSocketRecoveryAction::RETRY;
  case WebSocketErrorCategory::TIMEOUT:
    return WebSocketRecoveryAction::RETRY;
  case WebSocketErrorCategory::PROTOCOL:
    return (severity >= WebSocketErrorSeverity::HIGH)
               ? WebSocketRecoveryAction::RECONNECT
...
WebSocketError
WebSocketErrorFactory::createFromBoostError(const boost::system::error_code &ec,
...
WebSocketError
WebSocketErrorFactory::createFromException(const std::exception &ex,
                                           const std::string &component,
                                           const std::string &operation) {
  WebSocketError error;
  error.errorId = generateErrorId();
...
WebSocketError
WebSocketErrorFactory::createConnectionError(const std::string &message,
...
WebSocketError
WebSocketErrorFactory::createProtocolError(const std::string &message,
                                           const std::string &details) {
  WebSocketError error;
  error.errorId = generateErrorId();
...
WebSocketError
WebSocketErrorFactory::createTimeoutError(const std::string &operation,
...
WebSocketError
WebSocketErrorFactory::createMessageError(const std::string &message,
                                          const std::string &details) {
  WebSocketError error;
  error.errorId = generateErrorId();
...
WebSocketError
WebSocketErrorFactory::createAuthenticationError(const std::string &message,
                                                 const std::string &details) {
  WebSocketError error;
  error.errorId = generateErrorId();
...
WebSocketErrorCategory WebSocketErrorFactory::categorizeBoostError(
  ...
  if (ec == websocket::error::closed ||
      ...
      ec == websocket::error::bad_method || ec == websocket::error::no_host ||
...
WebSocketErrorFactory::determineSeverity(WebSocketErrorCategory category,
                                         const boost::system::error_code &ec) {
  switch (category) {
  case WebSocketErrorCategory::CONNECTION:
...
void WebSocketErrorRegistry::registerHandler(
    const std::string &component,
    std::shared_ptr<WebSocketErrorHandler> handler) {
  std::lock_guard<std::mutex> lock(handlersMutex_);
  handlers_[component] = handler;
  spdlog::debug(
      "WebSocketErrorRegistry: Registered error handler for component: {}",
      component);
}
...
void WebSocketErrorRegistry::unregisterHandler(const std::string &component) {
  ...
  if (it != handlers_.end()) {
    handlers_.erase(it);
    spdlog::debug(
        "WebSocketErrorRegistry: Unregistered error handler for component: {}",
        component);
  }
}

std::shared_ptr<WebSocketErrorHandler>
WebSocketErrorRegistry::getHandler(const std::string &component) {
  std::lock_guard<std::mutex> lock(handlersMutex_);
  auto it = handlers_.find(component);
  if (it != handlers_.end()) {
    return it->second;
...
void WebSocketErrorRegistry::handleGlobalError(const WebSocketError &error) {
  ...
  if (globalHandler_) {
    ...
    spdlog::error("Unhandled WebSocket error in {}: {}", error.component,
                  error.toString());
  }
}

void WebSocketErrorRegistry::setGlobalErrorHandler(
    std::shared_ptr<WebSocketErrorHandler> handler) {
  std::lock_guard<std::mutex> lock(handlersMutex_);
...
StandardWebSocketErrorHandler::ErrorStatistics
WebSocketErrorRegistry::getGlobalStatistics() const {
  std::lock_guard<std::mutex> lock(handlersMutex_);
  ...
  for (const auto &[component, handler] : handlers_) {
    if (auto standardHandler =
            std::dynamic_pointer_cast<StandardWebSocketErrorHandler>(handler)) {
  ...
  if (auto standardGlobalHandler =
          std::dynamic_pointer_cast<StandardWebSocketErrorHandler>(
              globalHandler_)) {
