...
std::string FifoLogEntry::toString() const {
    ...
    std::string levelStr;
    switch (level) {
        case FifoLogLevel::TRACE: levelStr = "TRACE"; break;
        case FifoLogLevel::DEBUG: levelStr = "DEBUG"; break;
        case FifoLogLevel::INFO: levelStr = "INFO"; break;
        case FifoLogLevel::WARN: levelStr = "WARN"; break;
        case FifoLogLevel::ERROR: levelStr = "ERROR"; break;
        case FifoLogLevel::CRITICAL: levelStr = "CRITICAL"; break;
...
std::string FifoLogEntry::toFormattedString() const {
    ...
    switch (level) {
        ...
        case FifoLogLevel::WARN: levelStr = "WARN "; break;
        case FifoLogLevel::ERROR: levelStr = "ERROR"; break;
        case FifoLogLevel::CRITICAL: levelStr = "CRIT "; break;
...
json FifoMetrics::toJson() const {
    ...
    j["minLatencyMicros"] = minLatencyMicros.load() == UINT64_MAX ? 0 : minLatencyMicros.load();
    j["uptimeMs"] = getUptime().count();
    
    auto now = std::chrono::system_clock::now();
    auto timeSinceReset = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastResetTime);
...
void FifoLoggerConfig::fromJson(const json& j) {
    if (j.contains("logLevel")) logLevel = static_cast<FifoLogLevel>(j["logLevel"]);
    if (j.contains("enableConsoleLogging")) enableConsoleLogging = j["enableConsoleLogging"];
    if (j.contains("enableFileLogging")) enableFileLogging = j["enableFileLogging"];
    if (j.contains("enableAsyncLogging")) enableAsyncLogging = j["enableAsyncLogging"];
    if (j.contains("enableJsonLogging")) enableJsonLogging = j["enableJsonLogging"];
    if (j.contains("logFilePath")) logFilePath = j["logFilePath"];
    if (j.contains("jsonLogFilePath")) jsonLogFilePath = j["jsonLogFilePath"];
    if (j.contains("maxLogFileSize")) maxLogFileSize = j["maxLogFileSize"];
    if (j.contains("maxLogFiles")) maxLogFiles = j["maxLogFiles"];
    if (j.contains("enableLogRotation")) enableLogRotation = j["enableLogRotation"];
    if (j.contains("enableMessageTracing")) enableMessageTracing = j["enableMessageTracing"];
    if (j.contains("enableMessageContent")) enableMessageContent = j["enableMessageContent"];
    if (j.contains("maxTraceEntries")) maxTraceEntries = j["maxTraceEntries"];
    if (j.contains("traceRetentionTime")) traceRetentionTime = std::chrono::milliseconds(j["traceRetentionTime"]);
    if (j.contains("enablePerformanceMetrics")) enablePerformanceMetrics = j["enablePerformanceMetrics"];
    if (j.contains("metricsUpdateInterval")) metricsUpdateInterval = std::chrono::milliseconds(j["metricsUpdateInterval"]);
    if (j.contains("enableLatencyTracking")) enableLatencyTracking = j["enableLatencyTracking"];
    if (j.contains("enableThroughputTracking")) enableThroughputTracking = j["enableThroughputTracking"];
    if (j.contains("enableDebugMode")) enableDebugMode = j["enableDebugMode"];
    if (j.contains("enableVerboseLogging")) enableVerboseLogging = j["enableVerboseLogging"];
    if (j.contains("enableStackTraces")) enableStackTraces = j["enableStackTraces"];
    if (j.contains("debugCategories")) debugCategories = j["debugCategories"];
    if (j.contains("excludeCategories")) excludeCategories = j["excludeCategories"];
    if (j.contains("includeOnlyCategories")) includeOnlyCategories = j["includeOnlyCategories"];
    if (j.contains("minLogLevel")) minLogLevel = static_cast<FifoLogLevel>(j["minLogLevel"]);
    if (j.contains("asyncQueueSize")) asyncQueueSize = j["asyncQueueSize"];
    if (j.contains("asyncFlushInterval")) asyncFlushInterval = std::chrono::milliseconds(j["asyncFlushInterval"]);
    if (j.contains("asyncWorkerThreads")) asyncWorkerThreads = j["asyncWorkerThreads"];
}

bool FifoLoggerConfig::validate() const {
    if (maxLogFileSize == 0) return false;
    if (maxLogFiles <= 0) return false;
    if (maxTraceEntries == 0) return false;
    if (asyncQueueSize == 0) return false;
    if (asyncWorkerThreads <= 0) return false;
    return true;
}
...
void FifoLogger::log(FifoLogLevel level, const std::string& category, const std::string& message, 
...
void FifoLogger::critical(const std::string& category, const std::string& message, 
                         const std::string& clientId, const std::string& pipePath) {
    log(FifoLogLevel::CRITICAL, category, message, clientId, pipePath);
}

void FifoLogger::traceMessage(const std::string& messageId, const std::string& clientId,
...
std::vector<FifoMessageTrace> FifoLogger::getMessageTraces(const std::string& clientId,
    ...
    while (!tempQueue.empty()) {
        ...
        if (maxAge.count() > 0) {
            auto age = std::chrono::duration_cast<std::chrono::milliseconds>(now - trace.timestamp);
            if (age > maxAge) {
                continue;
            }
        }
...
void FifoLogger::recordMessage(size_t bytes, std::chrono::microseconds latency) {
    ...
    if (config_.enableLatencyTracking && latency.count() > 0) {
        ...
        while (latency.count() > currentMax &&
               !metrics_.maxLatencyMicros.compare_exchange_weak(currentMax, latency.count())) {
        ...
        while (latency.count() < currentMin &&
               !metrics_.minLatencyMicros.compare_exchange_weak(currentMin, latency.count())) {
...
void FifoLogger::rotateLogs() {
    ...
    if (config_.enableFileLogging && getFileSize(config_.logFilePath) > config_.maxLogFileSize) {
        rotateLogFile(config_.logFilePath);
    }

    if (config_.enableJsonLogging && getFileSize(config_.jsonLogFilePath) > config_.maxLogFileSize) {
...
std::string FifoLogger::formatLogLevel(FifoLogLevel level) const {
    switch (level) {
        case FifoLogLevel::TRACE: return "TRACE";
        case FifoLogLevel::DEBUG: return "DEBUG";
        case FifoLogLevel::INFO: return "INFO";
        case FifoLogLevel::WARN: return "WARN";
        case FifoLogLevel::ERROR: return "ERROR";
        case FifoLogLevel::CRITICAL: return "CRITICAL";
        case FifoLogLevel::OFF: return "OFF";
...
void FifoLogger::setLogLevelForCategory(const std::string& category, FifoLogLevel level) {
    std::lock_guard<std::mutex> lock(categoryMutex_);
...
void FifoLogger::startAsyncLogging() {
    ...
    asyncThread_ = std::make_unique<std::thread>(&FifoLogger::asyncLoggingThread, this);
}
...
bool FifoLogger::isAsyncLoggingActive() const {
...
void FifoLogger::writeToFile(const FifoLogEntry& entry) {
    ...
    if (logFile_ && logFile_->is_open()) {
        *logFile_ << entry.toFormattedString() << std::endl;
        ...
        if (config_.enableLogRotation && getFileSize(config_.logFilePath) > config_.maxLogFileSize) {
            rotateLogFile(config_.logFilePath);
        }
    }
}
...
void FifoLogger::writeToJsonFile(const FifoLogEntry& entry) {
    ...
    if (jsonLogFile_ && jsonLogFile_->is_open()) {
        *jsonLogFile_ << entry.toJson().dump() << std::endl;
        ...
        // Check if rotation is needed
        if (config_.enableLogRotation && getFileSize(config_.jsonLogFilePath) > config_.maxLogFileSize) {
            rotateLogFile(config_.jsonLogFilePath);
        }
    }
}
...
bool FifoLogger::shouldLogCategory(const std::string& category) const {
    ...
    // Check exclude list
    if (std::find(config_.excludeCategories.begin(), config_.excludeCategories.end(), category)
    ...
    if (!config_.includeOnlyCategories.empty()) {
        return std::find(config_.includeOnlyCategories.begin(), config_.includeOnlyCategories.end(), category)
...
void FifoLogger::initializeLogFiles() {
    ...
    if (config_.enableFileLogging) {
        logFile_ = std::make_unique<std::ofstream>(config_.logFilePath, std::ios::app);
        if (!logFile_->is_open()) {
    ...
    if (config_.enableJsonLogging) {
        jsonLogFile_ = std::make_unique<std::ofstream>(config_.jsonLogFilePath, std::ios::app);
        if (!jsonLogFile_->is_open()) {
            std::cerr << "Failed to open JSON log file: " << config_.jsonLogFilePath << std::endl;
        }
    }
}
...
std::string FifoLogger::getLogFileName(const std::string& basePath, int index) const {
    if (index == 0) {
        return basePath;
    }
