...
void FifoConfig::fromJson(const json& j) {
    if (j.contains("pipeName")) pipeName = j["pipeName"];
    if (j.contains("pipeDirectory")) pipeDirectory = j["pipeDirectory"];
    if (j.contains("pipeType")) pipeType = static_cast<FifoPipeType>(j["pipeType"]);
    if (j.contains("accessMode")) accessMode = static_cast<FifoAccessMode>(j["accessMode"]);
    if (j.contains("unixPipePath")) unixPipePath = j["unixPipePath"];
    if (j.contains("windowsPipePath")) windowsPipePath = j["windowsPipePath"];
    
    if (j.contains("framingMode")) framingMode = static_cast<FifoFramingMode>(j["framingMode"]);
    if (j.contains("customDelimiter")) customDelimiter = j["customDelimiter"];
    if (j.contains("lineTerminator")) lineTerminator = j["lineTerminator"];
    if (j.contains("messagePrefix")) messagePrefix = j["messagePrefix"];
    if (j.contains("messageSuffix")) messageSuffix = j["messageSuffix"];
    
    if (j.contains("bufferSize")) bufferSize = j["bufferSize"];
    if (j.contains("maxMessageSize")) maxMessageSize = j["maxMessageSize"];
    if (j.contains("readBufferSize")) readBufferSize = j["readBufferSize"];
    if (j.contains("writeBufferSize")) writeBufferSize = j["writeBufferSize"];
    if (j.contains("maxQueueSize")) maxQueueSize = j["maxQueueSize"];
    
    if (j.contains("connectTimeout")) connectTimeout = std::chrono::milliseconds(j["connectTimeout"]);
    if (j.contains("readTimeout")) readTimeout = std::chrono::milliseconds(j["readTimeout"]);
    if (j.contains("writeTimeout")) writeTimeout = std::chrono::milliseconds(j["writeTimeout"]);
    if (j.contains("reconnectDelay")) reconnectDelay = std::chrono::milliseconds(j["reconnectDelay"]);
    if (j.contains("keepAliveInterval")) keepAliveInterval = std::chrono::milliseconds(j["keepAliveInterval"]);
    
    if (j.contains("maxReconnectAttempts")) maxReconnectAttempts = j["maxReconnectAttempts"];
    if (j.contains("enableAutoReconnect")) enableAutoReconnect = j["enableAutoReconnect"];
    ...
    if (j.contains("pipePermissions")) pipePermissions = j["pipePermissions"];
    if (j.contains("pipeOwner")) pipeOwner = j["pipeOwner"];
    if (j.contains("pipeGroup")) pipeGroup = j["pipeGroup"];
    
    if (j.contains("windowsPipeInstances")) windowsPipeInstances = j["windowsPipeInstances"];
    if (j.contains("windowsOutBufferSize")) windowsOutBufferSize = j["windowsOutBufferSize"];
    if (j.contains("windowsInBufferSize")) windowsInBufferSize = j["windowsInBufferSize"];
    if (j.contains("windowsDefaultTimeout")) windowsDefaultTimeout = j["windowsDefaultTimeout"];
    ...
    if (j.contains("enableMessageTracing")) enableMessageTracing = j["enableMessageTracing"];
    if (j.contains("enableBinaryMode")) enableBinaryMode = j["enableBinaryMode"];
    if (j.contains("enableFlowControl")) enableFlowControl = j["enableFlowControl"];
    if (j.contains("enableBackpressure")) enableBackpressure = j["enableBackpressure"];
    
    if (j.contains("compressionType")) compressionType = static_cast<FifoCompressionType>(j["compressionType"]);
    if (j.contains("compressionLevel")) compressionLevel = j["compressionLevel"];
    if (j.contains("compressionThreshold")) compressionThreshold = j["compressionThreshold"];
    if (j.contains("enableCompressionForSmallMessages")) enableCompressionForSmallMessages = j["enableCompressionForSmallMessages"];
    
    if (j.contains("authMethod")) authMethod = static_cast<FifoAuthMethod>(j["authMethod"]);
    if (j.contains("authToken")) authToken = j["authToken"];
    if (j.contains("certificatePath")) certificatePath = j["certificatePath"];
    if (j.contains("privateKeyPath")) privateKeyPath = j["privateKeyPath"];
    if (j.contains("enableEncryption")) enableEncryption = j["enableEncryption"];
    if (j.contains("encryptionKey")) encryptionKey = j["encryptionKey"];
    
    if (j.contains("enableCircuitBreaker")) enableCircuitBreaker = j["enableCircuitBreaker"];
    if (j.contains("circuitBreakerThreshold")) circuitBreakerThreshold = j["circuitBreakerThreshold"];
    if (j.contains("circuitBreakerTimeout")) circuitBreakerTimeout = std::chrono::milliseconds(j["circuitBreakerTimeout"]);
    if (j.contains("enableRetryOnError")) enableRetryOnError = j["enableRetryOnError"];
    if (j.contains("maxRetryAttempts")) maxRetryAttempts = j["maxRetryAttempts"];
    if (j.contains("retryDelay")) retryDelay = std::chrono::milliseconds(j["retryDelay"]);
    ...
    if (j.contains("enableDebugLogging")) enableDebugLogging = j["enableDebugLogging"];
    if (j.contains("logLevel")) logLevel = j["logLevel"];
    
    if (j.contains("enableMultiplexing")) enableMultiplexing = j["enableMultiplexing"];
    if (j.contains("maxConcurrentConnections")) maxConcurrentConnections = j["maxConcurrentConnections"];
    if (j.contains("enableMessagePrioritization")) enableMessagePrioritization = j["enableMessagePrioritization"];
    if (j.contains("enableMessageDeduplication")) enableMessageDeduplication = j["enableMessageDeduplication"];
    if (j.contains("deduplicationWindow")) deduplicationWindow = std::chrono::milliseconds(j["deduplicationWindow"]);
    
    if (j.contains("enablePlatformOptimizations")) enablePlatformOptimizations = j["enablePlatformOptimizations"];
    if (j.contains("useMemoryMappedFiles")) useMemoryMappedFiles = j["useMemoryMappedFiles"];
    if (j.contains("enableZeroCopy")) enableZeroCopy = j["enableZeroCopy"];
    if (j.contains("ioVectorSize")) ioVectorSize = j["ioVectorSize"];
}

bool FifoConfig::validate() const {
    // Basic validation
    if (pipeName.empty()) return false;
    if (bufferSize == 0 || maxMessageSize == 0) return false;
    if (readBufferSize == 0 || writeBufferSize == 0) return false;
    if (maxQueueSize == 0) return false;
    
    // Timeout validation
    if (connectTimeout.count() <= 0 || readTimeout.count() <= 0 || writeTimeout.count() <= 0) return false;
    
    // Path validation
    if (unixPipePath.empty() && windowsPipePath.empty()) return false;
...
FifoConfig FifoConfigManager::createConfig(ConfigPreset preset) const {
    switch (preset) {
        case ConfigPreset::DEFAULT: return createDefaultConfig();
        case ConfigPreset::HIGH_PERFORMANCE: return createHighPerformanceConfig();
        case ConfigPreset::LOW_LATENCY: return createLowLatencyConfig();
        case ConfigPreset::RELIABLE: return createReliableConfig();
        case ConfigPreset::SECURE: return createSecureConfig();
        case ConfigPreset::DEBUG: return createDebugConfig();
        case ConfigPreset::EMBEDDED: return createEmbeddedConfig();
        case ConfigPreset::BIDIRECTIONAL: return createBidirectionalConfig();
        case ConfigPreset::BROADCAST: return createBroadcastConfig();
        case ConfigPreset::STREAMING: return createStreamingConfig();
        default: return createDefaultConfig();
    }
}

FifoConfig FifoConfigManager::createCustomConfig(const json& customSettings) const {
    FifoConfig config = createDefaultConfig();
    config.fromJson(customSettings);
    return config;
}

FifoConfig FifoConfigManager::mergeConfigs(const FifoConfig& base, const FifoConfig& override) const {
...
FifoConfigValidationResult FifoConfigManager::validateConfig(const FifoConfig& config) const {
    ...
    if (config.maxMessageSize > config.bufferSize * 100) {
        result.warnings.push_back("Max message size is much larger than buffer size, may cause performance issues");
    }
    
    // Platform-specific validation
    if (!validatePlatformCompatibility(config)) {
        result.errors.push_back("Configuration is not compatible with current platform");
        result.isValid = false;
    }
    
    // Path validation
    if (!validatePipePath(config.unixPipePath) && !validatePipePath(config.windowsPipePath)) {
    ...
    if (result.isValid) {
        ...
        result.summary = "Configuration has " + std::to_string(result.errors.size()) + " errors";
    }
...
bool FifoConfigManager::isUnix() const {
    return !isWindows();
}
...
bool FifoConfigManager::validateTimeouts(const FifoConfig& config) const {
    return config.connectTimeout.count() > 0 &&
           config.readTimeout.count() > 0 &&
           config.writeTimeout.count() > 0 &&
           config.reconnectDelay.count() >= 0;
}
...
bool FifoConfigManager::validatePlatformCompatibility(const FifoConfig& config) const {
