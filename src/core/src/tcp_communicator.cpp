...
private:
    ...
    bool startServer() {
        tcp::endpoint endpoint(asio::ip::address::from_string(config_.bindInterface), config_.serverPort);
        ...
        SPDLOG_INFO("TCP server started on {}:{}", config_.bindInterface, config_.serverPort);
        return true;
    }
    ...
    bool startClient() {
        ...
        auto endpoints = resolver.resolve(config_.serverAddress, std::to_string(config_.serverPort));
        ...
        asio::async_connect(session->socket, endpoints,
            [this, session](std::error_code ec, tcp::endpoint) {
                if (!ec) {
                    ...
                    SPDLOG_INFO("TCP client connected to {}:{}", config_.serverAddress, config_.serverPort);
                } else {
    ...
    void startAccept() {
        ...
        acceptor_.async_accept(newSession->socket,
            [this, newSession](std::error_code ec) {
                if (!ec) {
                    std::lock_guard<std::mutex> lock(clientsMutex_);
    ...
    bool sendToSession(std::shared_ptr<ClientSession> session, const std::string& message) {
        try {
            std::size_t bytesWritten = asio::write(session->socket, asio::buffer(message));
            bytesSent_.fetch_add(bytesWritten);
            return true;
        } catch (const std::exception& e) {
            SPDLOG_ERROR("Failed to send to session {}: {}", session->clientId, e.what());
            return false;
        }
    }

    void handleDisconnection(std::shared_ptr<ClientSession> session, std::error_code ec) {
        SPDLOG_DEBUG("TCP client {} disconnected: {}", session->clientId, ec.message());
...
std::unique_ptr<TcpCommunicator> createTcpCommunicatorImpl(const TcpConfig& config) {
