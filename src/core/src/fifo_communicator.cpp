...
double FifoStatistics::getMessagesPerSecond() const {
    ...
    return static_cast<double>(messagesSent.load() + messagesReceived.load()) / duration.count();
}
...
FifoCommunicator::FifoCommunicator(const FifoConfig& config) : config_(config) {
    spdlog::debug("FifoCommunicator created with pipe: {}", 
                  ...
                  config_.windowsPipePath : config_.unixPipePath);
}
...
void FifoCommunicator::handleConnection(bool connected) {
    spdlog::info("FIFO connection state changed: {}", connected ? "connected" : "disconnected");
...
std::string FifoCommunicator::parseMessage(const std::string& rawMessage) const {
...
FifoCommunicatorImpl::FifoCommunicatorImpl(const FifoConfig& config) 
    : FifoCommunicator(config), lastReconnectAttempt_(std::chrono::system_clock::now()) {
...
bool FifoCommunicatorImpl::start() {
    ...
    readerThread_ = std::make_unique<std::thread>(&FifoCommunicatorImpl::readerThreadFunction, this);
    writerThread_ = std::make_unique<std::thread>(&FifoCommunicatorImpl::writerThreadFunction, this);
    
    if (config_.enableAutoReconnect) {
        reconnectThread_ = std::make_unique<std::thread>(&FifoCommunicatorImpl::reconnectThreadFunction, this);
    }
...
bool FifoCommunicatorImpl::isActive() const {
    return active_.load();
}
...
bool FifoCommunicatorImpl::sendMessage(const std::string& message) {
    ...
    if (message.length() > config_.maxMessageSize) {
        handleError("Message too large: " + std::to_string(message.length()) + " bytes");
        return false;
    }
...
bool FifoCommunicatorImpl::createWindowsNamedPipe() {
    ...
    readHandle_ = CreateNamedPipeA(
        ...
        openMode,
        pipeMode,
        config_.windowsPipeInstances,
        config_.windowsOutBufferSize,
        config_.windowsInBufferSize,
        config_.windowsDefaultTimeout,
        NULL
    );

    if (readHandle_ == INVALID_HANDLE_VALUE) {
        DWORD error = GetLastError();
        handleError("Failed to create Windows named pipe: " + std::to_string(error));
        return false;
    }
...
bool FifoCommunicatorImpl::openWindowsNamedPipe() {
    if (readHandle_ == INVALID_HANDLE_VALUE) {
        ...
        if (readHandle_ == INVALID_HANDLE_VALUE) {
            DWORD error = GetLastError();
            handleError("Failed to open Windows named pipe: " + std::to_string(error));
            return false;
        }
...
bool FifoCommunicatorImpl::readFromWindowsPipe(std::string& message) {
    ...
    if (config_.enableNonBlocking) {
        BOOL result = ReadFile(readHandle_, buffer.data(), buffer.size(), &bytesRead, &readOverlapped_);
        if (!result) {
            DWORD error = GetLastError();
            if (error == ERROR_IO_PENDING) {
                // Wait for completion
                if (WaitForSingleObject(readOverlapped_.hEvent, config_.readTimeout.count()) == WAIT_TIMEOUT) {
        ...
        BOOL result = ReadFile(readHandle_, buffer.data(), buffer.size(), &bytesRead, NULL);
        if (!result) {
            DWORD error = GetLastError();
            handleReadError("Windows pipe read error: " + std::to_string(error));
            return false;
...
bool FifoCommunicatorImpl::writeToWindowsPipe(const std::string& message) {
    ...
    if (config_.enableNonBlocking) {
        BOOL result = WriteFile(writeHandle_, message.data(), message.size(), &bytesWritten, &writeOverlapped_);
        if (!result) {
            DWORD error = GetLastError();
            if (error == ERROR_IO_PENDING) {
                // Wait for completion
                if (WaitForSingleObject(writeOverlapped_.hEvent, config_.writeTimeout.count()) == WAIT_TIMEOUT) {
                    CancelIo(writeHandle_);
                    return false;
                }
                GetOverlappedResult(writeHandle_, &writeOverlapped_, &bytesWritten, FALSE);
            } else {
        ...
        BOOL result = WriteFile(writeHandle_, message.data(), message.size(), &bytesWritten, NULL);
        if (!result) {
            DWORD error = GetLastError();
            handleWriteError("Windows pipe write error: " + std::to_string(error));
            return false;
...
bool FifoCommunicatorImpl::createUnixFifo() {
    ...
    if (mkfifo(pipePath.c_str(), config_.pipePermissions) == -1) {
        if (errno != EEXIST) {
            handleError("Failed to create Unix FIFO: " + std::string(strerror(errno)));
            return false;
        }
    }
...
bool FifoCommunicatorImpl::shouldReconnect() const {
    ...
    auto now = std::chrono::system_clock::now();
    auto timeSinceLastAttempt = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastReconnectAttempt_);
...
bool FifoCommunicatorImpl::isCircuitBreakerOpen() const {
    ...
    auto now = std::chrono::system_clock::now();
    auto timeSinceOpen = std::chrono::duration_cast<std::chrono::milliseconds>(now - circuitBreakerOpenTime_);

    return timeSinceOpen < config_.circuitBreakerTimeout;
}

// Message framing methods
std::string FifoCommunicatorImpl::frameMessage(const std::string& message) const {
    return formatMessage(message);
}

std::vector<std::string> FifoCommunicatorImpl::parseFramedMessages(const std::string& data) const {
...
bool FifoCommunicatorImpl::performHealthCheck() const {
    ...
    auto now = std::chrono::system_clock::now();
    auto timeSinceActivity = std::chrono::duration_cast<std::chrono::milliseconds>(now - statistics_.lastActivity);

    if (config_.enableKeepAlive && timeSinceActivity > config_.keepAliveInterval * 2) {
        return false;
    }
...
std::unique_ptr<FifoCommunicator> FifoCommunicatorFactory::create(const FifoConfig& config) {
...
std::unique_ptr<FifoCommunicator> FifoCommunicatorFactory::createWithPreset(FifoConfigManager::ConfigPreset preset) {
    auto& configManager = getGlobalFifoConfigManager();
    FifoConfig config = configManager.createConfig(preset);
    return create(config);
}

std::unique_ptr<FifoCommunicator> FifoCommunicatorFactory::createForWindows(const FifoConfig& config) {
    FifoConfig windowsConfig = config;
    windowsConfig.pipeType = FifoPipeType::WINDOWS_NAMED_PIPE;
    return create(windowsConfig);
}

std::unique_ptr<FifoCommunicator> FifoCommunicatorFactory::createForUnix(const FifoConfig& config) {
    FifoConfig unixConfig = config;
    unixConfig.pipeType = FifoPipeType::UNIX_FIFO;
    return create(unixConfig);
}

std::unique_ptr<FifoCommunicator> FifoCommunicatorFactory::createBidirectional(const FifoConfig& config) {
...
std::unique_ptr<FifoCommunicator> FifoCommunicatorFactory::createHighPerformance(const FifoConfig& config) {
    auto& configManager = getGlobalFifoConfigManager();
    FifoConfig perfConfig = configManager.mergeConfigs(config,
        configManager.createConfig(FifoConfigManager::ConfigPreset::HIGH_PERFORMANCE));
    return create(perfConfig);
}

std::unique_ptr<FifoCommunicator> FifoCommunicatorFactory::createReliable(const FifoConfig& config) {
    auto& configManager = getGlobalFifoConfigManager();
    FifoConfig reliableConfig = configManager.mergeConfigs(config,
