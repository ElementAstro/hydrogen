...
public:
    ...
    bool connect() override {
        ...
        try {
            SPDLOG_INFO("Connecting to MQTT broker {}:{}", config_.brokerHost, config_.brokerPort);
    ...
    bool publish(const std::string& topic, const std::string& message, int qos) override {
        if (!connected_.load()) {
    ...
    bool publish(const std::string& topic, const json& message, int qos) override {
    ...
    bool unsubscribe(const std::string& topic) override {
        ...
        try {
            ...
            SPDLOG_ERROR("Failed to unsubscribe from topic '{}': {}", topic, e.what());
            return false;
        }
    }
...
std::unique_ptr<MqttCommunicator> ProtocolCommunicatorFactory::createMqttCommunicator(const MqttConfig& config) {
