...
WebSocketConnection::WebSocketConnection(std::shared_ptr<WebSocketErrorHandler> errorHandler)
...
bool WebSocketConnection::connect(const ConnectionConfig& config) {
    ...
    try {
        ...
        if (success) {
            ...
            spdlog::info("WebSocketConnection: Connected to {}:{}", config_.host, config_.port);
        } else {
...
void WebSocketConnection::disconnect() {
    ...
    spdlog::info("WebSocketConnection: Disconnected from {}:{}", config_.host, config_.port);
}

bool WebSocketConnection::isConnected() const {
    return state_.load() == ConnectionState::CONNECTED && websocketHandle_ != nullptr;
}

ConnectionState WebSocketConnection::getState() const {
    return state_.load();
}
...
bool WebSocketConnection::receiveMessage(std::string& data) {
    ...
    try {
        ...
        if (dis(gen) <= 10) { // 1% chance of receiving a message
            ...
            simulatedMessage["timestamp"] = std::chrono::duration_cast<std::chrono::milliseconds>(
...
void WebSocketConnection::updateConfig(const ConnectionConfig& config) {
    bool needsReconnect = (config.host != config_.host || config.port != config_.port);
    config_ = config;
...
void WebSocketConnection::updateStatistics(bool sent, bool received, size_t bytes, bool error) {
    std::lock_guard<std::mutex> lock(statisticsMutex_);
    
    if (sent) statistics_.messagesSent++;
    if (received) statistics_.messagesReceived++;
    if (error) statistics_.errors++;
...
bool HttpConnection::connect(const ConnectionConfig& config) {
    ...
    try {
        ...
        if (success) {
            ...
            spdlog::info("HttpConnection: Connected to {}:{}", config_.host, config_.port);
        } else {
            state_.store(ConnectionState::CONNECTION_ERROR);
            updateStatistics(false, false, 0, true);
        }
...
void HttpConnection::disconnect() {
    ...
    spdlog::info("HttpConnection: Disconnected from {}:{}", config_.host, config_.port);
}
...
void HttpConnection::updateStatistics(bool sent, bool received, size_t bytes, bool error) {
    std::lock_guard<std::mutex> lock(statisticsMutex_);
...
bool GrpcConnection::connect(const ConnectionConfig& config) {
    ...
    try {
        ...
        if (success) {
            ...
            spdlog::info("GrpcConnection: Connected to {}:{}", config_.host, config_.port);
        } else {
            state_.store(ConnectionState::CONNECTION_ERROR);
            updateStatistics(false, false, 0, true);
        }
...
void GrpcConnection::disconnect() {
    ...
    spdlog::info("GrpcConnection: Disconnected from {}:{}", config_.host, config_.port);
}
...
ConnectionState GrpcConnection::getState() const {
    return state_.load();
}
...
void GrpcConnection::updateStatistics(bool sent, bool received, size_t bytes, bool error) {
    std::lock_guard<std::mutex> lock(statisticsMutex_);
...
std::string UnifiedConnectionManager::createConnection(const ConnectionConfig& config) {
    ...
    connectionInfo->converter.reset(converterRegistry.getConverter(config.protocol));
    ...
    spdlog::info("UnifiedConnectionManager: Created connection {} for protocol {}",
                 connectionId, static_cast<int>(config.protocol));

    return connectionId;
}

bool UnifiedConnectionManager::connectConnection(const std::string& connectionId) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);

    auto it = connections_.find(connectionId);
    if (it == connections_.end()) {
        spdlog::error("UnifiedConnectionManager: Connection {} not found", connectionId);
        return false;
    }
    ...
    if (success) {
        ...
        handleConnectionStateChange(connectionId, ConnectionState::CONNECTION_ERROR);
    }

    return success;
}

void UnifiedConnectionManager::disconnectConnection(const std::string& connectionId) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
...
bool UnifiedConnectionManager::isConnected(const std::string& connectionId) const {
    std::lock_guard<std::mutex> lock(connectionsMutex_);

    auto it = connections_.find(connectionId);
    return (it != connections_.end()) ? it->second->connection->isConnected() : false;
}

ConnectionState UnifiedConnectionManager::getConnectionState(const std::string& connectionId) const {
    std::lock_guard<std::mutex> lock(connectionsMutex_);

    auto it = connections_.find(connectionId);
    return (it != connections_.end()) ? it->second->connection->getState() : ConnectionState::DISCONNECTED;
}

std::vector<std::string> UnifiedConnectionManager::getActiveConnections() const {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
...
bool UnifiedConnectionManager::sendMessage(const std::string& connectionId, std::shared_ptr<Message> message) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
    ...
    if (!it->second->connection->isConnected()) {
        spdlog::warn("UnifiedConnectionManager: Connection {} is not connected", connectionId);
        return false;
    }
    ...
    try {
        ...
        if (!result.success) {
            spdlog::error("UnifiedConnectionManager: Failed to transform message: {}", result.errorMessage);
            return false;
        }
        ...
        spdlog::error("UnifiedConnectionManager: Failed to send message on {}: {}", connectionId, e.what());
        return false;
    }
}

bool UnifiedConnectionManager::sendRawMessage(const std::string& connectionId, const std::string& data) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
...
bool UnifiedConnectionManager::broadcastMessage(std::shared_ptr<Message> message, const std::vector<std::string>& connectionIds) {
...
void UnifiedConnectionManager::updateConnectionConfig(const std::string& connectionId, const ConnectionConfig& config) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
...
ConnectionConfig UnifiedConnectionManager::getConnectionConfig(const std::string& connectionId) const {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
...
void UnifiedConnectionManager::enableAutoReconnect(const std::string& connectionId, bool enable) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);

    auto it = connections_.find(connectionId);
    if (it != connections_.end()) {
        it->second->autoReconnect.store(enable);
        spdlog::debug("UnifiedConnectionManager: Auto-reconnect {} for connection {}",
                     enable ? "enabled" : "disabled", connectionId);
    }
}

void UnifiedConnectionManager::setReconnectSettings(const std::string& connectionId, std::chrono::milliseconds interval, int maxAttempts) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
    ...
    if (it != connections_.end()) {
        ...
        spdlog::debug("UnifiedConnectionManager: Updated reconnect settings for {}: interval={}ms, maxAttempts={}",
                     connectionId, interval.count(), maxAttempts);
    }
}

ConnectionStatistics UnifiedConnectionManager::getConnectionStatistics(const std::string& connectionId) const {
    std::lock_guard<std::mutex> lock(connectionsMutex_);

    auto it = connections_.find(connectionId);
    return (it != connections_.end()) ? it->second->connection->getStatistics() : ConnectionStatistics{};
}

std::unordered_map<std::string, ConnectionStatistics> UnifiedConnectionManager::getAllStatistics() const {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
...
void UnifiedConnectionManager::resetStatistics(const std::string& connectionId) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
    ...
    if (connectionId.empty()) {
        ...
        spdlog::debug("UnifiedConnectionManager: Reset statistics for all connections");
    } else {
        auto it = connections_.find(connectionId);
        if (it != connections_.end()) {
            it->second->connection->resetStatistics();
            spdlog::debug("UnifiedConnectionManager: Reset statistics for connection {}", connectionId);
        }
    }
}

std::vector<MessageFormat> UnifiedConnectionManager::getSupportedProtocols() const {
    return {
...
bool UnifiedConnectionManager::isProtocolSupported(MessageFormat protocol) const {
...
void UnifiedConnectionManager::startMessageProcessing() {
    ...
    messageProcessingThread_ = std::thread(&UnifiedConnectionManager::messageProcessingLoop, this);
...
std::unique_ptr<ProtocolConnection> UnifiedConnectionManager::createProtocolConnection(MessageFormat protocol) {
    switch (protocol) {
        ...
        try {
            ...
            for (const auto& connectionId : activeConnections) {
...

                {
                    std::lock_guard<std::mutex> lock(connectionsMutex_);
                    auto it = connections_.find(connectionId);
                    if (it != connections_.end() && it->second->connection->receiveMessage(messageData)) {
                        try {
                            ...
                            spdlog::error("UnifiedConnectionManager: Failed to parse message from {}: {}",
                                         connectionId, e.what());
                        }
                    }
                }
            ...
            spdlog::error("UnifiedConnectionManager: Error in message processing loop: {}", e.what());
    ...
    while (!shutdown_.load()) {
        try {
            ...
            std::vector<std::string> connectionsToCheck;
            {
                std::lock_guard<std::mutex> lock(connectionsMutex_);
                for (const auto& [connectionId, connectionInfo] : connections_) {
                    if (!connectionInfo->connection->isConnected() && connectionInfo->autoReconnect.load()) {
            ...
            spdlog::error("UnifiedConnectionManager: Error in reconnection loop: {}", e.what());
...
void UnifiedConnectionManager::handleConnectionStateChange(const std::string& connectionId, ConnectionState newState) {
    if (connectionCallback_) {
        try {
            connectionCallback_(connectionId, newState);
        } catch (const std::exception& e) {
            spdlog::error("UnifiedConnectionManager: Error in connection callback: {}", e.what());
        }
    }
}

void UnifiedConnectionManager::attemptReconnection(const std::string& connectionId) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
    ...
    if (connectionInfo->config.maxReconnectAttempts > 0 &&
        ...
        spdlog::warn("UnifiedConnectionManager: Max reconnect attempts exceeded for {}", connectionId);
        return;
    }
    ...
    spdlog::info("UnifiedConnectionManager: Attempting reconnection for {} (attempt {})",
    ...
    if (success) {
        ...
        handleConnectionStateChange(connectionId, ConnectionState::CONNECTED);
        spdlog::info("UnifiedConnectionManager: Reconnection successful for {}", connectionId);
    } else {
        handleConnectionStateChange(connectionId, ConnectionState::CONNECTION_ERROR);
    }
}

std::unique_ptr<WebSocketConnection> UnifiedConnectionManager::createWebSocketConnection() {
    return std::make_unique<WebSocketConnection>();
}

std::unique_ptr<HttpConnection> UnifiedConnectionManager::createHttpConnection() {
    return std::make_unique<HttpConnection>();
}

std::unique_ptr<GrpcConnection> UnifiedConnectionManager::createGrpcConnection() {
    return std::make_unique<GrpcConnection>();
}

// ConnectionManagerFactory implementation
std::unique_ptr<UnifiedConnectionManager> ConnectionManagerFactory::createManager() {
    return std::make_unique<UnifiedConnectionManager>();
}

std::unique_ptr<UnifiedConnectionManager> ConnectionManagerFactory::createManagerWithDefaults() {
    auto manager = createManager();
    manager->startMessageProcessing();
    return manager;
}

ConnectionConfig ConnectionManagerFactory::createWebSocketConfig(const std::string& host, uint16_t port) {
...
ConnectionConfig ConnectionManagerFactory::createHttpConfig(const std::string& host, uint16_t port) {
    ConnectionConfig config;
    config.protocol = MessageFormat::HTTP_JSON;
    config.host = host;
    config.port = port;
...
ConnectionConfig ConnectionManagerFactory::createGrpcConfig(const std::string& host, uint16_t port) {
    ConnectionConfig config;
...
ConnectionConfig ConnectionManagerFactory::createMqttConfig(const std::string& host, uint16_t port) {
    ConnectionConfig config;
...
ConnectionConfig ConnectionManagerFactory::getSecureConfig(MessageFormat protocol, const std::string& host, uint16_t port) {
    ConnectionConfig config;
...
ConnectionConfig ConnectionManagerFactory::getHighPerformanceConfig(MessageFormat protocol, const std::string& host, uint16_t port) {
    ConnectionConfig config;
    config.protocol = protocol;
    config.host = host;
    config.port = port;
...
ConnectionConfig ConnectionManagerFactory::getReliableConfig(MessageFormat protocol, const std::string& host, uint16_t port) {
    ConnectionConfig config;
    config.protocol = protocol;
    config.host = host;
    config.port = port;
