...
UnifiedDeviceClient::UnifiedDeviceClient(const ClientConnectionConfig& config) 
    ...
    spdlog::info("UnifiedDeviceClient: Initialized with host {}:{}", config_.host, config_.port);
}

UnifiedDeviceClient::~UnifiedDeviceClient() {
    disconnect();
}
    ...
    try {
        ...
        if (success) {
            ...
            if (config_.heartbeatInterval.count() > 0) {
                heartbeatThread_ = std::thread(&UnifiedDeviceClient::heartbeatLoop, this);
            }
            
            if (config_.enableAutoReconnect) {
                reconnectionThread_ = std::thread(&UnifiedDeviceClient::reconnectionLoop, this);
            }
            
            notifyConnectionChange(true);
            spdlog::info("UnifiedDeviceClient: Successfully connected to {}:{}", host, port);
        } else {
            spdlog::error("UnifiedDeviceClient: Failed to connect to {}:{}", host, port);
...
void UnifiedDeviceClient::disconnect() {
    ...
    spdlog::info("UnifiedDeviceClient: Disconnecting from {}:{}", config_.host, config_.port);
    ...
    spdlog::info("UnifiedDeviceClient: Disconnected from {}:{}", config_.host, config_.port);
}
...
void UnifiedDeviceClient::setAutoReconnect(bool enable, std::chrono::milliseconds interval, int maxAttempts) {
    config_.enableAutoReconnect = enable;
    config_.reconnectInterval = interval;
    config_.maxReconnectAttempts = maxAttempts;
    
    spdlog::debug("UnifiedDeviceClient: Auto-reconnect {}, interval: {}ms, max attempts: {}", 
...
json UnifiedDeviceClient::getDeviceProperties(const std::string& deviceId, const std::vector<std::string>& properties) {
    if (!isConnected()) {
        throw std::runtime_error("Not connected to server");
    }
...
json UnifiedDeviceClient::setDeviceProperties(const std::string& deviceId, const json& properties) {
    if (!isConnected()) {
        throw std::runtime_error("Not connected to server");
    }
...
json UnifiedDeviceClient::executeCommand(const std::string& deviceId, const std::string& command, 
...
void UnifiedDeviceClient::executeCommandAsync(const std::string& deviceId, const std::string& command,
                                             const json& parameters, Message::QoSLevel qosLevel,
...
json UnifiedDeviceClient::executeBatchCommands(const std::string& deviceId,
                                              const std::vector<std::pair<std::string, json>>& commands,
                                              bool sequential, Message::QoSLevel qosLevel) {
    if (!isConnected()) {
        throw std::runtime_error("Not connected to server");
    }
    ...
    return sendMessage(std::move(batchCommand), config_.messageTimeout * commands.size());
}

json UnifiedDeviceClient::sendMessage(std::shared_ptr<Message> message, std::chrono::milliseconds timeout) {
    if (!isConnected()) {
        throw std::runtime_error("Not connected to server");
    }
    ...
    try {
        ...
        if (!result.success) {
            throw std::runtime_error("Failed to transform message: " + result.errorMessage);
        }
...
void UnifiedDeviceClient::sendMessageAsync(std::shared_ptr<Message> message, std::function<void(const json&)> callback) {
    if (!isConnected()) {
        if (callback) {
            json error;
            error["error"] = "Not connected to server";
...
bool UnifiedDeviceClient::subscribeToProperty(const std::string& deviceId, const std::string& property) {
    ...
    try {
        ...
        spdlog::error("UnifiedDeviceClient: Failed to subscribe to property {}.{}: {}", deviceId, property, e.what());
        return false;
    }
}

bool UnifiedDeviceClient::unsubscribeFromProperty(const std::string& deviceId, const std::string& property) {
    std::lock_guard<std::mutex> lock(subscriptionsMutex_);
    ...
    try {
        ...
        spdlog::error("UnifiedDeviceClient: Failed to unsubscribe from property {}.{}: {}", deviceId, property, e.what());
        return false;
    }
}

bool UnifiedDeviceClient::subscribeToEvents(const std::string& deviceId, const std::string& eventType) {
    std::lock_guard<std::mutex> lock(subscriptionsMutex_);
    ...
    try {
        ...
        spdlog::error("UnifiedDeviceClient: Failed to subscribe to events for {}: {}", deviceId, e.what());
        return false;
    }
}

bool UnifiedDeviceClient::unsubscribeFromEvents(const std::string& deviceId, const std::string& eventType) {
    std::lock_guard<std::mutex> lock(subscriptionsMutex_);
    auto& events = eventSubscriptions_[deviceId];
    events.erase(std::remove(events.begin(), events.end(), eventType), events.end());
    ...
    try {
        ...
        spdlog::error("UnifiedDeviceClient: Failed to unsubscribe from events for {}: {}", deviceId, e.what());
        return false;
    }
}
...
void UnifiedDeviceClient::startMessageProcessing() {
    ...
    messageProcessingThread_ = std::thread(&UnifiedDeviceClient::messageProcessingLoop, this);
...
bool UnifiedDeviceClient::establishConnection() {
    try {
        ...
        spdlog::error("UnifiedDeviceClient: Failed to establish connection: {}", e.what());
        return false;
    }
}
...
void UnifiedDeviceClient::cleanupConnection() {
    try {
        ...
        spdlog::error("UnifiedDeviceClient: Error during connection cleanup: {}", e.what());
    }
}
...
bool UnifiedDeviceClient::attemptReconnection() {
    ...
    if (config_.maxReconnectAttempts > 0 && attempts >= config_.maxReconnectAttempts) {
        spdlog::error("UnifiedDeviceClient: Maximum reconnection attempts ({}) exceeded", config_.maxReconnectAttempts);
        return false;
    }

    auto now = std::chrono::system_clock::now();
    auto timeSinceLastAttempt = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastReconnectionAttempt_);
    ...
    spdlog::info("UnifiedDeviceClient: Attempting reconnection {} of {}",
                 attempts + 1, config_.maxReconnectAttempts > 0 ? std::to_string(config_.maxReconnectAttempts) : "unlimited");
        ...
        try {
            ...
            if (websocketReceive(messageData)) {
                try {
                    ...
                    spdlog::error("UnifiedDeviceClient: Failed to parse incoming message: {}", e.what());
                    updateStatistics(false, false, true);
                }
            } else {
            ...
            spdlog::error("UnifiedDeviceClient: Error in message processing loop: {}", e.what());
            updateStatistics(false, false, true);
    ...
    while (!shutdown_.load()) {
        ...
        if (isConnected()) {
            try {
                ...
                spdlog::warn("UnifiedDeviceClient: Failed to send heartbeat: {}", e.what());
            }
        }
    }
...
void UnifiedDeviceClient::handleIncomingMessage(const json& message) {
    try {
        ...
        spdlog::error("UnifiedDeviceClient: Error handling incoming message: {}", e.what());
    }
}
...
void UnifiedDeviceClient::handleResponse(const json& response) {
    ...
    if (callbackIt != asyncCallbacks_.end()) {
        try {
            ...
            spdlog::error("UnifiedDeviceClient: Error in async callback: {}", e.what());
        }
        asyncCallbacks_.erase(callbackIt);
        return;
    }

    spdlog::debug("UnifiedDeviceClient: Received response for unknown message: {}", messageId);
}
...
void UnifiedDeviceClient::handleEvent(const json& event) {
    if (eventCallback_) {
        ...
        try {
            ...
            spdlog::error("UnifiedDeviceClient: Error in event callback: {}", e.what());
        }
    }
}
...
void UnifiedDeviceClient::handlePropertyChange(const json& propertyChange) {
    if (propertyChangeCallback_) {
        ...
        try {
            ...
            spdlog::error("UnifiedDeviceClient: Error in property change callback: {}", e.what());
        }
    }
}
...
void UnifiedDeviceClient::updateStatistics(bool sent, bool received, bool error) {
    std::lock_guard<std::mutex> lock(statisticsMutex_);
...
void UnifiedDeviceClient::notifyConnectionChange(bool connected) {
    if (connectionCallback_) {
        try {
            ...
            spdlog::error("UnifiedDeviceClient: Error in connection callback: {}", e.what());
        }
    }
}
...
void UnifiedDeviceClient::notifyError(const std::string& error) {
    if (errorCallback_) {
        try {
            ...
            spdlog::error("UnifiedDeviceClient: Error in error callback: {}", e.what());
        }
    }
}
...
bool UnifiedDeviceClient::websocketSend(const std::string& data) {
    ...
    if (!websocketConnection_) return false;
...
std::unique_ptr<UnifiedDeviceClient> UnifiedDeviceClientFactory::createClient(const ClientConnectionConfig& config) {
    return std::make_unique<UnifiedDeviceClient>(config);
}

std::unique_ptr<UnifiedDeviceClient> UnifiedDeviceClientFactory::createAndConnect(const std::string& host, uint16_t port) {
...
std::unique_ptr<UnifiedDeviceClient> UnifiedDeviceClientFactory::createWebSocketClient(const std::string& host, uint16_t port) {
...
std::unique_ptr<UnifiedDeviceClient> UnifiedDeviceClientFactory::createHttpClient(const std::string& host, uint16_t port) {
    ClientConnectionConfig config = getDefaultConfig();
    config.host = host;
    config.port = port;
    config.endpoint = "/api";
...
ClientConnectionConfig UnifiedDeviceClientFactory::getSecureConfig(const std::string& host, uint16_t port) {
    ClientConnectionConfig config = getDefaultConfig();
    config.host = host;
    config.port = port;
    config.useTls = true;
...
ClientConnectionConfig UnifiedDeviceClientFactory::getHighPerformanceConfig(const std::string& host, uint16_t port) {
    ClientConnectionConfig config = getDefaultConfig();
    config.host = host;
    config.port = port;
