#include "hydrogen/core/device/device_plugin.h"
...
json PluginApiVersion::toJson() const {
  return json{{"major", major}, {"minor", minor}, {"patch", patch}};
}
...
PluginMetadata PluginMetadata::fromJson(const json &j) {
  ...
  metadata.supportedDeviceTypes =
      j.value("supportedDeviceTypes", std::vector<std::string>{});
...
PluginSecurityContext PluginSecurityContext::fromJson(const json &j) {
  ...
  context.allowedNetworkHosts =
      j.value("allowedNetworkHosts", std::vector<std::string>{});

  if (j.contains("permissions") && j["permissions"].is_array()) {
    for (const auto &permissionStr : j["permissions"]) {
      context.permissions.push_back(
          stringToPluginPermission(permissionStr.get<std::string>()));
    }
  }
...
PluginValidationResult PluginValidationResult::fromJson(const json &j) {
  ...
  if (j.contains("securityContext")) {
    result.securityContext =
        PluginSecurityContext::fromJson(j["securityContext"]);
  }
...
bool DevicePluginManager::loadPlugin(const std::string &pluginPath) {
  ...
  if (!plugin->instance->initialize(plugin->securityContext,
...
std::shared_ptr<IDevice> DevicePluginManager::createDevice(
    const std::string &pluginId, const std::string &deviceType,
...
PluginMetadata
DevicePluginManager::getPluginMetadata(const std::string &pluginId) const {
  std::lock_guard<std::mutex> lock(pluginsMutex_);
...
std::vector<std::string> DevicePluginManager::getSupportedDeviceTypes(
...
DevicePluginManager::validatePlugin(const std::string &pluginPath) const {
  PluginValidationResult result;
...
DevicePluginManager::getAllPluginHealth() const {
  std::lock_guard<std::mutex> lock(pluginsMutex_);
...
PluginSecurityContext DevicePluginManager::createSecurityContext(
...
bool DevicePluginManager::validateSecurity(
    const PluginMetadata &metadata,
...
std::string
DevicePluginManager::generatePluginId(const PluginMetadata &metadata) const {
  return metadata.name + "_" + metadata.version;
}

bool DevicePluginManager::loadPluginLibrary(const std::string &pluginPath,
                                            LoadedPlugin &plugin) {
...
std::string pluginPermissionToString(PluginPermission permission) {
  switch (permission) {
  case PluginPermission::DEVICE_ACCESS:
    return "DEVICE_ACCESS";
  case PluginPermission::NETWORK_ACCESS:
    return "NETWORK_ACCESS";
  case PluginPermission::FILE_SYSTEM_READ:
    return "FILE_SYSTEM_READ";
  case PluginPermission::FILE_SYSTEM_WRITE:
    return "FILE_SYSTEM_WRITE";
  case PluginPermission::SYSTEM_COMMANDS:
    return "SYSTEM_COMMANDS";
  case PluginPermission::CONFIGURATION_READ:
    return "CONFIGURATION_READ";
  case PluginPermission::CONFIGURATION_WRITE:
    return "CONFIGURATION_WRITE";
  case PluginPermission::LOGGING_ACCESS:
    return "LOGGING_ACCESS";
  case PluginPermission::METRICS_ACCESS:
...
PluginPermission stringToPluginPermission(const std::string &permission) {
  if (permission == "DEVICE_ACCESS")
    return PluginPermission::DEVICE_ACCESS;
  if (permission == "NETWORK_ACCESS")
    return PluginPermission::NETWORK_ACCESS;
  if (permission == "FILE_SYSTEM_READ")
    return PluginPermission::FILE_SYSTEM_READ;
  if (permission == "FILE_SYSTEM_WRITE")
    return PluginPermission::FILE_SYSTEM_WRITE;
  if (permission == "SYSTEM_COMMANDS")
    return PluginPermission::SYSTEM_COMMANDS;
  if (permission == "CONFIGURATION_READ")
    return PluginPermission::CONFIGURATION_READ;
  if (permission == "CONFIGURATION_WRITE")
    return PluginPermission::CONFIGURATION_WRITE;
  if (permission == "LOGGING_ACCESS")
    return PluginPermission::LOGGING_ACCESS;
  if (permission == "METRICS_ACCESS")
    return PluginPermission::METRICS_ACCESS;
  return PluginPermission::DEVICE_ACCESS;
}
