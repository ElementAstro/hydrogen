...
StateTransition StateTransition::fromJson(const json &j) {
  ...
  transition.fromState =
      stringToLifecycleState(j.value("fromState", "UNINITIALIZED"));
  transition.toState =
      stringToLifecycleState(j.value("toState", "UNINITIALIZED"));
...
LifecycleEvent LifecycleEvent::fromJson(const json &j) {
  ...
  event.previousState =
      stringToLifecycleState(j.value("previousState", "UNINITIALIZED"));
...
DeviceLifecycleManager::getCurrentState(const std::string &deviceId) const {
  std::lock_guard<std::mutex> lock(lifecycleMutex_);
...
bool DeviceLifecycleManager::isValidTransition(
    const std::string & /* deviceId */, DeviceLifecycleState fromState,
    DeviceLifecycleState toState) const {
  return isTransitionAllowed(fromState, toState);
}

std::vector<DeviceLifecycleState>
DeviceLifecycleManager::getValidNextStates(const std::string &deviceId) const {
  std::lock_guard<std::mutex> lock(lifecycleMutex_);
...
DeviceLifecycleManager::getStateHistory(const std::string &deviceId,
  ...
  return std::vector<StateTransition>(history.end() - maxEntries,
                                      history.end());
}
...
void DeviceLifecycleManager::forceErrorState(const std::string &deviceId,
...
bool DeviceLifecycleManager::attemptRecovery(const std::string &deviceId) {
  return transitionTo(deviceId, DeviceLifecycleState::RECOVERING,
                      "RECOVERY_ATTEMPT", "Attempting automatic recovery");
}

std::vector<std::string>
DeviceLifecycleManager::getDevicesInState(DeviceLifecycleState state) const {
  std::lock_guard<std::mutex> lock(lifecycleMutex_);
...
std::string lifecycleStateToString(DeviceLifecycleState state) {
  ...
  case DeviceLifecycleState::INITIALIZING:
    return "INITIALIZING";
  case DeviceLifecycleState::INITIALIZED:
    return "INITIALIZED";
  case DeviceLifecycleState::CONNECTING:
    return "CONNECTING";
  case DeviceLifecycleState::CONNECTED:
    return "CONNECTED";
  case DeviceLifecycleState::STARTING:
    return "STARTING";
  case DeviceLifecycleState::RUNNING:
    return "RUNNING";
  case DeviceLifecycleState::PAUSING:
    return "PAUSING";
  case DeviceLifecycleState::PAUSED:
    return "PAUSED";
  case DeviceLifecycleState::RESUMING:
    return "RESUMING";
  case DeviceLifecycleState::STOPPING:
    return "STOPPING";
  case DeviceLifecycleState::STOPPED:
    return "STOPPED";
  case DeviceLifecycleState::DISCONNECTING:
    return "DISCONNECTING";
  case DeviceLifecycleState::DISCONNECTED:
    return "DISCONNECTED";
  case DeviceLifecycleState::ERROR:
    return "ERROR";
  case DeviceLifecycleState::RECOVERING:
    return "RECOVERING";
  case DeviceLifecycleState::MAINTENANCE:
    return "MAINTENANCE";
  case DeviceLifecycleState::UPDATING:
    return "UPDATING";
  case DeviceLifecycleState::SHUTDOWN:
    return "SHUTDOWN";
  default:
    return "UNKNOWN";
  }
...
DeviceLifecycleState stringToLifecycleState(const std::string &state) {
  ...
  if (state == "INITIALIZING")
    return DeviceLifecycleState::INITIALIZING;
  if (state == "INITIALIZED")
    return DeviceLifecycleState::INITIALIZED;
  if (state == "CONNECTING")
    return DeviceLifecycleState::CONNECTING;
  if (state == "CONNECTED")
    return DeviceLifecycleState::CONNECTED;
  if (state == "STARTING")
    return DeviceLifecycleState::STARTING;
  if (state == "RUNNING")
    return DeviceLifecycleState::RUNNING;
  if (state == "PAUSING")
    return DeviceLifecycleState::PAUSING;
  if (state == "PAUSED")
    return DeviceLifecycleState::PAUSED;
  if (state == "RESUMING")
    return DeviceLifecycleState::RESUMING;
  if (state == "STOPPING")
    return DeviceLifecycleState::STOPPING;
  if (state == "STOPPED")
    return DeviceLifecycleState::STOPPED;
  if (state == "DISCONNECTING")
    return DeviceLifecycleState::DISCONNECTING;
  if (state == "DISCONNECTED")
    return DeviceLifecycleState::DISCONNECTED;
  if (state == "ERROR")
    return DeviceLifecycleState::ERROR;
  if (state == "RECOVERING")
    return DeviceLifecycleState::RECOVERING;
  if (state == "MAINTENANCE")
    return DeviceLifecycleState::MAINTENANCE;
  if (state == "UPDATING")
    return DeviceLifecycleState::UPDATING;
  if (state == "SHUTDOWN")
    return DeviceLifecycleState::SHUTDOWN;
  return DeviceLifecycleState::UNINITIALIZED;
}
...
void DeviceLifecycleManager::initializeValidTransitions() {
  ...
  validTransitions_[DeviceLifecycleState::UNINITIALIZED] = {
      DeviceLifecycleState::INITIALIZING, DeviceLifecycleState::ERROR};

  validTransitions_[DeviceLifecycleState::INITIALIZING] = {
      DeviceLifecycleState::INITIALIZED, DeviceLifecycleState::ERROR};

  validTransitions_[DeviceLifecycleState::INITIALIZED] = {
      DeviceLifecycleState::CONNECTING, DeviceLifecycleState::ERROR,
      DeviceLifecycleState::SHUTDOWN};

  validTransitions_[DeviceLifecycleState::CONNECTING] = {
      DeviceLifecycleState::CONNECTED, DeviceLifecycleState::ERROR,
      DeviceLifecycleState::DISCONNECTED};

  validTransitions_[DeviceLifecycleState::CONNECTED] = {
      DeviceLifecycleState::STARTING, DeviceLifecycleState::DISCONNECTING,
      DeviceLifecycleState::ERROR, DeviceLifecycleState::MAINTENANCE};
  ...
  validTransitions_[DeviceLifecycleState::RUNNING] = {
      DeviceLifecycleState::PAUSING, DeviceLifecycleState::STOPPING,
      DeviceLifecycleState::ERROR, DeviceLifecycleState::MAINTENANCE,
      DeviceLifecycleState::UPDATING};

  validTransitions_[DeviceLifecycleState::PAUSING] = {
      DeviceLifecycleState::PAUSED, DeviceLifecycleState::ERROR};

  validTransitions_[DeviceLifecycleState::PAUSED] = {
      DeviceLifecycleState::RESUMING, DeviceLifecycleState::STOPPING,
  ...
  validTransitions_[DeviceLifecycleState::STOPPING] = {
      DeviceLifecycleState::STOPPED, DeviceLifecycleState::ERROR};
  ...
  validTransitions_[DeviceLifecycleState::DISCONNECTING] = {
      DeviceLifecycleState::DISCONNECTED, DeviceLifecycleState::ERROR};

  validTransitions_[DeviceLifecycleState::DISCONNECTED] = {
      DeviceLifecycleState::CONNECTING, DeviceLifecycleState::SHUTDOWN,
      DeviceLifecycleState::ERROR};

  validTransitions_[DeviceLifecycleState::ERROR] = {
      DeviceLifecycleState::RECOVERING, DeviceLifecycleState::SHUTDOWN,
      DeviceLifecycleState::MAINTENANCE};

  validTransitions_[DeviceLifecycleState::RECOVERING] = {
      DeviceLifecycleState::INITIALIZED, DeviceLifecycleState::CONNECTED,
...
bool DeviceLifecycleManager::isTransitionAllowed(
    DeviceLifecycleState from, DeviceLifecycleState to) const {
...
void DeviceLifecycleManager::addToHistory(const std::string &deviceId,
...
void DeviceLifecycleManager::trimHistory(
...
bool DeviceLifecycleManager::saveLifecycleData(
  ...
  try {
    ...
    if (data.contains("devices")) {
      for (const auto &[deviceId, deviceData] : data["devices"].items()) {
        DeviceLifecycleInfo info;
        info.currentState = stringToLifecycleState(
            deviceData.value("currentState", "UNINITIALIZED"));
        info.lastErrorReason = deviceData.value("lastErrorReason", "");

        if (deviceData.contains("registrationTime")) {
          info.registrationTime =
              string_utils::parseIsoTimestamp(deviceData["registrationTime"]);
        }

        if (deviceData.contains("history") &&
            deviceData["history"].is_array()) {
