...
GrpcStreamingClient::GrpcStreamingClient(const GrpcConfig& config) : config_(config) {
    stats_.lastActivity = std::chrono::steady_clock::now();
}

GrpcStreamingClient::~GrpcStreamingClient() {
    disconnect();
}
...
void GrpcStreamingClient::disconnect() {
    ...
    spdlog::info("GrpcStreamingClient: Disconnecting from {}", config_.serverAddress);
...
bool GrpcStreamingClient::isConnected() const {
    return connected_.load();
}

CommunicationResponse GrpcStreamingClient::sendUnaryMessage(const Message& message) {
    ...
    try {
        ...
        grpc::ClientContext context;
        context.set_deadline(std::chrono::system_clock::now() + std::chrono::seconds(30)); // Default timeout
        
        // astrocomm::proto::Message protoResponse;
        // grpc::Status status = stub_->SendMessage(&context, protoMessage, &protoResponse);
...
bool GrpcStreamingClient::startClientStreaming(const std::string& method) {
    if (!connected_.load()) {
        spdlog::error("GrpcStreamingClient: Cannot start client streaming - not connected");
        return false;
    }
    ...
    try {
        ...
        clientStreamContext_->set_deadline(std::chrono::system_clock::now() + std::chrono::hours(1));
        
        // astrocomm::proto::Message response;
        // clientStream_ = stub_->ClientStreamingMethod(clientStreamContext_.get(), &response);
        ...
        if (clientStream_) {
            ...
            std::thread(&GrpcStreamingClient::processClientStreamQueue, this).detach();
            
            spdlog::info("GrpcStreamingClient: Started client streaming for method: {}", method);
            return true;
        } else {
        ...
        spdlog::error("GrpcStreamingClient: Failed to start client streaming: {}", e.what());
...
bool GrpcStreamingClient::sendClientStreamMessage(const Message& message) {
    ...
    try {
        ...
        spdlog::error("GrpcStreamingClient: Failed to queue client stream message: {}", e.what());
...
CommunicationResponse GrpcStreamingClient::finishClientStreaming() {
    ...
    try {
        ...
        response.success = false;
        response.errorMessage = "Failed to finish client streaming: " + std::string(e.what());
...
bool GrpcStreamingClient::startServerStreaming(const std::string& method, const Message& request) {
    if (!connected_.load()) {
        spdlog::error("GrpcStreamingClient: Cannot start server streaming - not connected");
        return false;
    }
    ...
    try {
        ...
        serverStreamContext_->set_deadline(std::chrono::system_clock::now() + std::chrono::hours(1));
        
        auto protoRequest = convertToProtoMessage(request);
        // serverStream_ = stub_->ServerStreamingMethod(serverStreamContext_.get(), protoRequest);
        ...
        if (serverStream_) {
            ...
            serverStreamThread_ = std::thread(&GrpcStreamingClient::serverStreamingLoop, this);
            
            spdlog::info("GrpcStreamingClient: Started server streaming for method: {}", method);
            return true;
        } else {
        ...
        spdlog::error("GrpcStreamingClient: Failed to start server streaming: {}", e.what());
...
bool GrpcStreamingClient::startBidirectionalStreaming(const std::string& method) {
    if (!connected_.load()) {
        spdlog::error("GrpcStreamingClient: Cannot start bidirectional streaming - not connected");
        return false;
    }
    ...
    try {
        ...
        bidirectionalStreamContext_->set_deadline(std::chrono::system_clock::now() + std::chrono::hours(1));
        
        // bidirectionalStream_ = stub_->BidirectionalStreamingMethod(bidirectionalStreamContext_.get());
        ...
        if (bidirectionalStream_) {
            ...
            bidirectionalStreamThread_ = std::thread(&GrpcStreamingClient::bidirectionalStreamingLoop, this);
            
            spdlog::info("GrpcStreamingClient: Started bidirectional streaming for method: {}", method);
            return true;
        } else {
            spdlog::error("GrpcStreamingClient: Failed to create bidirectional stream");
            return false;
        }
        
    } catch (const std::exception& e) {
        spdlog::error("GrpcStreamingClient: Failed to start bidirectional streaming: {}", e.what());
...
bool GrpcStreamingClient::sendBidirectionalMessage(const Message& message) {
    ...
    try {
        ...
        if (success) {
            ...
            spdlog::error("GrpcStreamingClient: Failed to write to bidirectional stream");
            incrementStreamingErrors();
            return false;
        }
        
    } catch (const std::exception& e) {
        spdlog::error("GrpcStreamingClient: Failed to send bidirectional message: {}", e.what());
...
void GrpcStreamingClient::updateConfig(const GrpcConfig& config) {
    config_ = config;
    if (connected_.load()) {
        spdlog::info("GrpcStreamingClient: Configuration updated, reconnection may be required");
    }
}
...
bool GrpcStreamingClient::initializeChannel() {
    try {
        ...
        auto deadline = std::chrono::system_clock::now() + std::chrono::seconds(30); // Default timeout
        ...
        spdlog::error("GrpcStreamingClient: Failed to initialize channel: {}", e.what());
        return false;
    }
}
...
void GrpcStreamingClient::serverStreamingLoop() {
    ...
    while (serverStreaming_.load() && !shutdown_.load()) {
        try {
            ...
            if (messageHandler_) {
                auto simulatedMessage = std::make_unique<EventMessage>("server_stream_data");
                simulatedMessage->setDetails(json{{"timestamp", std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            spdlog::error("GrpcStreamingClient: Error in server streaming loop: {}", e.what());
...
void GrpcStreamingClient::bidirectionalStreamingLoop() {
    ...
    while (bidirectionalStreaming_.load() && !shutdown_.load()) {
        try {
            ...
            // Simulate receiving a message
            if (messageHandler_) {
                auto simulatedMessage = std::make_unique<EventMessage>("bidirectional_stream_data");
                simulatedMessage->setDetails(json{{"timestamp", std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::system_clock::now().time_since_epoch()).count()}});
                messageHandler_(*simulatedMessage);
                incrementMessagesReceived();
            ...
            spdlog::error("GrpcStreamingClient: Error in bidirectional streaming loop: {}", e.what());
    ...
    while (clientStreaming_.load() && !shutdown_.load()) {
        try {
            ...
            clientStreamCV_.wait(lock, [this] {
                return !clientStreamQueue_.empty() || !clientStreaming_.load() || shutdown_.load();
            });
            ...
            while (!clientStreamQueue_.empty() && clientStreaming_.load()) {
                ...
                if (!success) {
                    spdlog::error("GrpcStreamingClient: Failed to write to client stream");
                    incrementStreamingErrors();
                    break;
                }
            ...
            spdlog::error("GrpcStreamingClient: Error in client stream queue processor: {}", e.what());
...
std::unique_ptr<Message> GrpcStreamingClient::convertFromProtoMessage(const json& protoMessage) const {
...
void GrpcStreamingClient::handleStreamError(const std::string& error, const std::string& context) {
...
std::unique_ptr<GrpcStreamingClient> GrpcStreamingFactory::createStreamingClient(const GrpcConfig& config) {
    return std::make_unique<GrpcStreamingClient>(config);
}

std::unique_ptr<GrpcStreamingServer> GrpcStreamingFactory::createStreamingServer(const GrpcConfig& config) {
