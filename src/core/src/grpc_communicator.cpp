...
public:
  GrpcCommunicatorImpl(const GrpcConfig &config) : GrpcCommunicator(config) {
    SPDLOG_INFO("gRPC Communicator initialized for server {}",
                config_.serverAddress);
  }

  ~GrpcCommunicatorImpl() { disconnect(); }
  ...
  bool connect() override {
    ...
    try {
      ...
      //     channel_ = grpc::CreateCustomChannel(config_.serverAddress, creds,
      //     args);
      // } else {
  ...
  sendUnaryRequest(const CommunicationMessage &message) override {
    ...
    try {
      ...
      // grpc::Status status = stub_->SendCommand(&context, request,
      // &grpcResponse);
  ...
  bool startServerStreaming(const std::string &method,
                            const CommunicationMessage &request) override {
    if (!connected_.load()) {
    ...
    try {
      ...
      streamThread_ =
          std::thread(&GrpcCommunicatorImpl::serverStreamingLoop, this);
  ...
  bool startBidirectionalStreaming(const std::string &method) override {
    ...
    try {
      ...
      streamThread_ =
          std::thread(&GrpcCommunicatorImpl::bidirectionalStreamingLoop, this);
...
std::unique_ptr<GrpcCommunicator>
ProtocolCommunicatorFactory::createGrpcCommunicator(const GrpcConfig &config) {
