...
ZmqQueueClient::ZmqQueueClient(const ZmqClientConfig& config) : config_(config) {
    statistics_.lastMessageTime = std::chrono::steady_clock::now();
    lastReconnectAttempt_ = std::chrono::steady_clock::now();
}

ZmqQueueClient::~ZmqQueueClient() {
    disconnect();
}

bool ZmqQueueClient::connect() {
    if (connected_.load()) {
        spdlog::warn("ZmqQueueClient: Already connected to {}", config_.serverAddress);
        return true;
    }
    ...
    try {
        ...
        messageProcessorThread_ = std::thread(&ZmqQueueClient::messageProcessorLoop, this);
        ...
        spdlog::info("ZmqQueueClient: Successfully connected to {}", config_.serverAddress);
...
bool ZmqQueueClient::isConnected() const {
...
bool ZmqQueueClient::reconnect() {
    ...
    spdlog::info("ZmqQueueClient: Reconnection attempt {} of {}", 
                 attempts + 1, config_.maxReconnectAttempts);
    
    return connect();
}

bool ZmqQueueClient::sendMessage(const std::string& content, ZmqMessageType type) {
    ZmqMessage message;
    message.id = generateMessageId();
    message.content = content;
    message.clientId = "client_" + std::to_string(reinterpret_cast<uintptr_t>(this));
...
bool ZmqQueueClient::sendMessageSync(const std::string& content, std::string& response, int timeoutMs) {
    if (!connected_.load()) {
    ...
    if (!received) {
        spdlog::warn("ZmqQueueClient: Synchronous message timeout for ID: {}", message.id);
    }
...
void ZmqQueueClient::enqueueMessage(const ZmqMessage& message) {
    {
        ...
        if (outgoingQueue_.size() >= static_cast<size_t>(config_.highWaterMark)) {
            spdlog::warn("ZmqQueueClient: Outgoing queue full, dropping oldest message");
    ...
    spdlog::debug("ZmqQueueClient: Message queued for sending: {} (queue size: {})",
...
void ZmqQueueClient::flushOutgoingQueue() {
    ...
    spdlog::info("ZmqQueueClient: Flushing outgoing queue ({} messages)", outgoingQueue_.size());
...
void ZmqQueueClient::clearQueue() {
    ...
    spdlog::info("ZmqQueueClient: All queues cleared (outgoing: {}, incoming: {})",
...
bool ZmqQueueClient::setSocketOption(int option, int value) {
    ...
    spdlog::debug("ZmqQueueClient: Setting socket option {} to {}", option, value);
    return true;
}
...
bool ZmqQueueClient::initializeSocket() {
    try {
        ...
        // 
        // // Set socket options
        // zmq_setsockopt(socket_, ZMQ_SNDHWM, &config_.highWaterMark, sizeof(config_.highWaterMark));
        // zmq_setsockopt(socket_, ZMQ_RCVHWM, &config_.highWaterMark, sizeof(config_.highWaterMark));
        // zmq_setsockopt(socket_, ZMQ_LINGER, &config_.lingerTime, sizeof(config_.lingerTime));
        // zmq_setsockopt(socket_, ZMQ_RCVTIMEO, &config_.receiveTimeout, sizeof(config_.receiveTimeout));
        // zmq_setsockopt(socket_, ZMQ_SNDTIMEO, &config_.sendTimeout, sizeof(config_.sendTimeout));
        // 
        // // Connect to server
...
void ZmqQueueClient::messageProcessorLoop() {
    ...
    while (running_.load()) {
        try {
            ...
            outgoingQueueCV_.wait(lock, [this] {
                return !outgoingQueue_.empty() || !running_.load();
            });

            // Process queued messages in batches to match server behavior
            std::vector<ZmqMessage> messageBatch;
            size_t batchSize = std::min(static_cast<size_t>(10), outgoingQueue_.size());
            ...
            for (const auto& message : messageBatch) {
                ...
                if (sent) {
                    ...
                    spdlog::debug("ZmqQueueClient: Message sent successfully: {}", message.id);
                } else {
                    spdlog::warn("ZmqQueueClient: Failed to send message: {}", message.id);
            ...
            spdlog::error("ZmqQueueClient: Error in message processor loop: {}", e.what());
        ...
        try {
            ...
            if (simulateReceive(receivedData)) {
                try {
                    ...
                    {
                        std::lock_guard<std::mutex> lock(incomingQueueMutex_);
                        if (incomingQueue_.size() >= static_cast<size_t>(config_.highWaterMark)) {
                            spdlog::warn("ZmqQueueClient: Incoming queue full, dropping oldest message");
                    ...
                    spdlog::debug("ZmqQueueClient: Message received and queued: {} (queue size: {})",
                                  message.id, getQueueSize());

                } catch (const json::parse_error& e) {
                    spdlog::error("ZmqQueueClient: Failed to parse received message: {}", e.what());
...
void ZmqQueueClient::heartbeatLoop() {
    ...
    while (running_.load()) {
        try {
            std::this_thread::sleep_for(std::chrono::milliseconds(config_.heartbeatInterval));
...
bool ZmqQueueClient::sendQueuedMessage(const ZmqMessage& message) {
    ...
    try {
        ...
        spdlog::error("ZmqQueueClient: Failed to serialize message {}: {}", message.id, e.what());
        return false;
    }
}
...
void ZmqQueueClient::processIncomingMessage(const ZmqMessage& message) {
    ...
    switch (message.type) {
        ...
        case ZmqMessageType::BROADCAST:
            ...
            break;
        default:
            spdlog::warn("ZmqQueueClient: Unknown message type: {}", static_cast<int>(message.type));
            break;
    }
...
void ZmqQueueClient::handleHeartbeatMessage(const ZmqMessage& message) {
    spdlog::trace("ZmqQueueClient: Received heartbeat from server: {}", message.id);
    ...
    if (config_.enableHeartbeat) {
        ...
        response.clientId = "client_" + std::to_string(reinterpret_cast<uintptr_t>(this));
...
void ZmqQueueClient::handleControlMessage(const ZmqMessage& message) {
    spdlog::debug("ZmqQueueClient: Received control message: {}", message.content);
    ...
    try {
        ...
        spdlog::error("ZmqQueueClient: Failed to parse control message: {}", e.what());
    }
}
...
void ZmqQueueClient::handleSynchronousResponse(const ZmqMessage& message) {
    ...
    std::string originalMessageId = message.metadata.value("originalMessageId", "");
...
ZmqMessage ZmqQueueClient::createHeartbeatMessage() const {
    ...
    heartbeat.clientId = "client_" + std::to_string(reinterpret_cast<uintptr_t>(this));
...
void ZmqQueueClient::updateStatistics(bool sent, bool received, bool queued, bool processed) {
    std::lock_guard<std::mutex> lock(statisticsMutex_);

    if (sent) statistics_.totalMessagesSent++;
    if (received) statistics_.totalMessagesReceived++;
    if (queued) statistics_.totalMessagesQueued++;
    if (processed) statistics_.totalMessagesProcessed++;
...
void ZmqQueueClient::updateConnectionStatus(bool connected) {
    if (connectionHandler_) {
        try {
            ...
            spdlog::error("ZmqQueueClient: Error in connection handler: {}", e.what());
        }
    }
}
...
std::unique_ptr<ZmqQueueClient> ZmqQueueClientFactory::createClient(const ZmqClientConfig& config) {
    return std::make_unique<ZmqQueueClient>(config);
}

std::unique_ptr<ZmqQueueClient> ZmqQueueClientFactory::createReqClient(const std::string& serverAddress) {
    ZmqClientConfig config = getDefaultConfig(ZmqSocketType::REQ);
    config.serverAddress = serverAddress;
    return createClient(config);
}

std::unique_ptr<ZmqQueueClient> ZmqQueueClientFactory::createSubClient(const std::string& serverAddress, const std::string& topic) {
...
std::unique_ptr<ZmqQueueClient> ZmqQueueClientFactory::createPullClient(const std::string& serverAddress) {
...
ZmqClientConfig ZmqQueueClientFactory::getDefaultConfig(ZmqSocketType socketType) {
...
void ZmqQueueManager::registerClient(const std::string& clientId, std::shared_ptr<ZmqQueueClient> client) {
...
std::shared_ptr<ZmqQueueClient> ZmqQueueManager::getClient(const std::string& clientId) {
    std::lock_guard<std::mutex> lock(clientsMutex_);
    auto it = clients_.find(clientId);
    ...
    for (const auto& [clientId, client] : clients_) {
        if (client && client->isConnected()) {
            if (!client->sendMessage(message)) {
                spdlog::warn("ZmqQueueManager: Failed to broadcast message to client: {}", clientId);
                allSent = false;
            }
        }
    }
...
std::vector<std::pair<std::string, ZmqClientStatistics>> ZmqQueueManager::getAllClientStatistics() const {
    std::lock_guard<std::mutex> lock(clientsMutex_);
