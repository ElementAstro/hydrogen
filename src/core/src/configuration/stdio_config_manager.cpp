...
bool StdioConfigManager::validateBasicSettings(const StdioConfig &config, std::string &error) const {
  if (config.bufferSize == 0) {
    error = "Buffer size cannot be zero";
    return false;
  }
  if (config.bufferSize > 1024 * 1024) { // 1MB limit
    error = "Buffer size too large (max 1MB)";
    return false;
  }
  if (config.readTimeout.count() < 0) {
    error = "Read timeout cannot be negative";
    return false;
  }
  if (config.writeTimeout.count() < 0) {
    error = "Write timeout cannot be negative";
    return false;
  }
  return true;
}

bool StdioConfigManager::validateFramingSettings(const StdioConfig &config, std::string &error) const {
  // Basic framing validation - could be extended based on actual framing modes
  return true;
}

bool StdioConfigManager::validateCompressionSettings(const StdioConfig &config, std::string &error) const {
  // Basic compression validation - could be extended based on actual compression settings
  return true;
}

bool StdioConfigManager::validateSecuritySettings(const StdioConfig &config, std::string &error) const {
  // Basic security validation - could be extended based on actual security settings
  return true;
}

bool StdioConfigManager::validateConnectionSettings(const StdioConfig &config, std::string &error) const {
  // Basic connection validation - could be extended based on actual connection settings
  return true;
}

bool StdioConfigManager::validatePerformanceSettings(const StdioConfig &config, std::string &error) const {
  // Basic performance validation - could be extended based on actual performance settings
  return true;
}
