...
void NetworkConfig::fromJson(const json& j) {
    ...
    keepAliveInterval = std::chrono::milliseconds(j.value("keepAliveInterval", 30000));
    maxConnections = j.value("maxConnections", 100);
    maxReconnectAttempts = j.value("maxReconnectAttempts", 0);
    reconnectInterval = std::chrono::milliseconds(j.value("reconnectInterval", 5000));
...
void DiscoveryConfig::fromJson(const json& j) {
    ...
    discoveryInterval = std::chrono::milliseconds(j.value("discoveryInterval", 30000));
...
ConfigValidationResult ClientConfiguration::validate() const {
    ...
    // Merge results
    result.errors.insert(result.errors.end(), networkResult.errors.begin(), networkResult.errors.end());
    result.errors.insert(result.errors.end(), authResult.errors.begin(), authResult.errors.end());
    result.errors.insert(result.errors.end(), loggingResult.errors.begin(), loggingResult.errors.end());
    result.errors.insert(result.errors.end(), performanceResult.errors.begin(), performanceResult.errors.end());
    result.errors.insert(result.errors.end(), discoveryResult.errors.begin(), discoveryResult.errors.end());

    result.warnings.insert(result.warnings.end(), networkResult.warnings.begin(), networkResult.warnings.end());
    result.warnings.insert(result.warnings.end(), authResult.warnings.begin(), authResult.warnings.end());
    ...
    result.warnings.insert(result.warnings.end(), discoveryResult.warnings.begin(), discoveryResult.warnings.end());

    result.isValid = result.errors.empty();

    // Cross-validation checks
    if (network.useTls && authentication.type == AuthConfig::AuthType::BASIC) {
        result.addWarning("Using basic authentication over TLS - consider using stronger authentication");
    }

    if (performance.enableCompression && network.useTls) {
        result.addWarning("Compression with TLS may be vulnerable to CRIME/BREACH attacks");
    }
...
bool ClientConfiguration::loadFromFile(const std::string& filePath) {
    try {
        ...
        spdlog::error("ClientConfiguration: Failed to load from file {}: {}", filePath, e.what());
        return false;
    }
}
...
bool ClientConfiguration::saveToFile(const std::string& filePath) const {
    try {
        ...
        spdlog::error("ClientConfiguration: Failed to save to file {}: {}", filePath, e.what());
        return false;
    }
}
...
bool ClientConfiguration::loadFromString(const std::string& jsonString) {
    try {
        ...
        spdlog::error("ClientConfiguration: Failed to parse JSON string: {}", e.what());
        return false;
    }
}
...
std::string ClientConfiguration::saveToString() const {
    try {
        ...
        spdlog::error("ClientConfiguration: Failed to serialize to JSON: {}", e.what());
        return "{}";
    }
}
...
void ClientConfiguration::merge(const ClientConfiguration& other) {
    ...
    if (!other.network.host.empty()) network.host = other.network.host;
    if (other.network.port != 8080) network.port = other.network.port;
    if (!other.network.endpoint.empty()) network.endpoint = other.network.endpoint;
...
void ClientConfiguration::enableFeature(const std::string& feature, bool enable) {
    features[feature] = enable;
    spdlog::debug("ClientConfiguration: Feature '{}' {}", feature, enable ? "enabled" : "disabled");
}
...
void ClientConfiguration::setProtocolConfig(MessageFormat protocol, const json& config) {
    protocolConfigs[protocol] = config;
}
...
void ClientConfiguration::processEnvironmentVariables(const std::string& prefix) {
    // Network settings
    ...
    if (const char* useTls = std::getenv((prefix + "USE_TLS").c_str())) {
        network.useTls = (std::string(useTls) == "true" || std::string(useTls) == "1");
    }
    ...
    if (const char* logLevel = std::getenv((prefix + "LOG_LEVEL").c_str())) {
        ...
        if (level == "TRACE") logging.level = LoggingConfig::LogLevel::TRACE;
        else if (level == "DEBUG") logging.level = LoggingConfig::LogLevel::DEBUG;
        else if (level == "INFO") logging.level = LoggingConfig::LogLevel::INFO;
        else if (level == "WARN") logging.level = LoggingConfig::LogLevel::WARN;
        else if (level == "ERROR") logging.level = LoggingConfig::LogLevel::ERR;
        else if (level == "CRITICAL") logging.level = LoggingConfig::LogLevel::CRITICAL;
    }
}

std::string ClientConfiguration::expandEnvironmentVariables(const std::string& value) const {
...
void ConfigurationManager::registerConfiguration(const std::string& name, std::shared_ptr<ClientConfiguration> config) {
...
std::shared_ptr<ClientConfiguration> ConfigurationManager::getConfiguration(const std::string& name) const {
    std::lock_guard<std::mutex> lock(configMutex_);
    auto it = configurations_.find(name);
    return (it != configurations_.end()) ? it->second : nullptr;
}

std::shared_ptr<ClientConfiguration> ConfigurationManager::getDefaultConfiguration() const {
...
void ConfigurationManager::setDefaultConfiguration(const std::string& name) {
    ...
    if (configurations_.find(name) != configurations_.end()) {
        ...
        spdlog::info("ConfigurationManager: Set default configuration to '{}'", name);
    } else {
        spdlog::warn("ConfigurationManager: Cannot set default to non-existent configuration '{}'", name);
    }
}
...
bool ConfigurationManager::loadConfiguration(const std::string& name, const std::string& filePath) {
...
bool ConfigurationManager::saveConfiguration(const std::string& name, const std::string& filePath) const {
...
std::unordered_map<std::string, ConfigValidationResult> ConfigurationManager::validateAllConfigurations() const {
    std::lock_guard<std::mutex> lock(configMutex_);
...
void ConfigurationManager::watchConfigurationFile(const std::string& name, const std::string& filePath) {
    std::lock_guard<std::mutex> lock(configMutex_);
    watchedFiles_[name] = filePath;

    if (!fileWatcherThread_.joinable()) {
        stopWatching_.store(false);
        fileWatcherThread_ = std::thread(&ConfigurationManager::fileWatcherLoop, this);
    }
}
                ...
                if (file.good()) {
                    ...
                    if (timeT > lastModified[filePath]) {
                        ...
                        if (config && config->loadFromFile(filePath)) {
                            ...
                            spdlog::info("ConfigurationManager: Reloaded configuration '{}' from {}", name, filePath);
                        }
                    }
                }
            } catch (const std::exception& e) {
                spdlog::error("ConfigurationManager: Error watching file {}: {}", filePath, e.what());
            }
        }
    }
}
...
ConfigurationBuilder::ConfigurationBuilder() : config_(std::make_unique<ClientConfiguration>()) {
}
...
ConfigurationBuilder& ConfigurationBuilder::withEndpoint(const std::string& endpoint) {
...
ConfigurationBuilder& ConfigurationBuilder::withTimeout(std::chrono::milliseconds timeout) {
...
ConfigurationBuilder& ConfigurationBuilder::withBasicAuth(const std::string& username, const std::string& password) {
...
ConfigurationBuilder& ConfigurationBuilder::withBearerToken(const std::string& token) {
    config_->authentication.type = AuthConfig::AuthType::BEARER_TOKEN;
    config_->authentication.token = token;
    return *this;
}

ConfigurationBuilder& ConfigurationBuilder::withApiKey(const std::string& key, const std::string& header) {
...
ConfigurationBuilder& ConfigurationBuilder::withProtocol(MessageFormat protocol) {
    config_->defaultProtocol = protocol;
    return *this;
}

ConfigurationBuilder& ConfigurationBuilder::withProtocolConfig(MessageFormat protocol, const json& configJson) {
    config_->protocolConfigs[protocol] = configJson;
    return *this;
}

ConfigurationBuilder& ConfigurationBuilder::withFeature(const std::string& feature, bool enable) {
...
ConfigurationBuilder& ConfigurationBuilder::withLogLevel(LoggingConfig::LogLevel level) {
    config_->logging.level = level;
    return *this;
}

ConfigurationBuilder& ConfigurationBuilder::withLogFile(const std::string& filePath) {
