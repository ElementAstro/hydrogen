...
std::string StdioConfigManager::getValidationError(const StdioConfig& config) const {
    std::string error;
    
    if (!validateBasicSettings(config, error)) return error;
    if (!validateFramingSettings(config, error)) return error;
    if (!validateCompressionSettings(config, error)) return error;
    if (!validateSecuritySettings(config, error)) return error;
    if (!validateConnectionSettings(config, error)) return error;
    if (!validatePerformanceSettings(config, error)) return error;
...
StdioConfig StdioConfigManager::configFromJson(const json& j) const {
    ...
    // Basic I/O settings
    if (j.contains("enableLineBuffering")) config.enableLineBuffering = j["enableLineBuffering"];
    if (j.contains("enableBinaryMode")) config.enableBinaryMode = j["enableBinaryMode"];
    ...
    if (j.contains("lineTerminator")) config.lineTerminator = j["lineTerminator"];
    if (j.contains("enableEcho")) config.enableEcho = j["enableEcho"];
    if (j.contains("enableFlush")) config.enableFlush = j["enableFlush"];
    if (j.contains("encoding")) config.encoding = j["encoding"];
    if (j.contains("enableErrorRedirection")) config.enableErrorRedirection = j["enableErrorRedirection"];
    
    // Message framing
    if (j.contains("framingMode")) config.framingMode = framingModeFromJson(j["framingMode"]);
    if (j.contains("frameDelimiter")) config.frameDelimiter = j["frameDelimiter"];
    if (j.contains("enableFrameEscaping")) config.enableFrameEscaping = j["enableFrameEscaping"];
    if (j.contains("escapeSequence")) config.escapeSequence = j["escapeSequence"];
    if (j.contains("maxFrameSize")) config.maxFrameSize = j["maxFrameSize"];
    
    // Compression
    if (j.contains("enableCompression")) config.enableCompression = j["enableCompression"];
    if (j.contains("compressionType")) config.compressionType = compressionTypeFromJson(j["compressionType"]);
    if (j.contains("compressionLevel")) config.compressionLevel = j["compressionLevel"];
    if (j.contains("compressionThreshold")) config.compressionThreshold = j["compressionThreshold"];
    
    // Authentication and security
    if (j.contains("enableAuthentication")) config.enableAuthentication = j["enableAuthentication"];
    if (j.contains("authenticationMethod")) config.authenticationMethod = j["authenticationMethod"];
    ...
    if (j.contains("enableEncryption")) config.enableEncryption = j["enableEncryption"];
    if (j.contains("encryptionKey")) config.encryptionKey = j["encryptionKey"];
    if (j.contains("encryptionAlgorithm")) config.encryptionAlgorithm = j["encryptionAlgorithm"];
    
    // Connection management
    if (j.contains("enableHeartbeat")) config.enableHeartbeat = j["enableHeartbeat"];
    if (j.contains("heartbeatInterval")) config.heartbeatInterval = std::chrono::milliseconds(j["heartbeatInterval"]);
    if (j.contains("heartbeatTimeout")) config.heartbeatTimeout = std::chrono::milliseconds(j["heartbeatTimeout"]);
    if (j.contains("enableReconnect")) config.enableReconnect = j["enableReconnect"];
    if (j.contains("maxReconnectAttempts")) config.maxReconnectAttempts = j["maxReconnectAttempts"];
    if (j.contains("reconnectDelay")) config.reconnectDelay = std::chrono::milliseconds(j["reconnectDelay"]);
    if (j.contains("maxReconnectDelay")) config.maxReconnectDelay = std::chrono::milliseconds(j["maxReconnectDelay"]);
    if (j.contains("enableBackoffStrategy")) config.enableBackoffStrategy = j["enableBackoffStrategy"];
    
    // Flow control
    if (j.contains("enableFlowControl")) config.enableFlowControl = j["enableFlowControl"];
    if (j.contains("sendBufferSize")) config.sendBufferSize = j["sendBufferSize"];
    if (j.contains("receiveBufferSize")) config.receiveBufferSize = j["receiveBufferSize"];
    if (j.contains("maxPendingMessages")) config.maxPendingMessages = j["maxPendingMessages"];
    if (j.contains("enableBackpressure")) config.enableBackpressure = j["enableBackpressure"];
    
    // Protocol-specific settings
    if (j.contains("enableMessageValidation")) config.enableMessageValidation = j["enableMessageValidation"];
    if (j.contains("enableMessageTransformation")) config.enableMessageTransformation = j["enableMessageTransformation"];
    if (j.contains("enableMessageLogging")) config.enableMessageLogging = j["enableMessageLogging"];
    if (j.contains("logLevel")) config.logLevel = j["logLevel"];
    if (j.contains("enableMetrics")) config.enableMetrics = j["enableMetrics"];
    if (j.contains("enableTracing")) config.enableTracing = j["enableTracing"];
    
    // Performance tuning
    if (j.contains("ioThreads")) config.ioThreads = j["ioThreads"];
    if (j.contains("enableNonBlockingIO")) config.enableNonBlockingIO = j["enableNonBlockingIO"];
    if (j.contains("enableZeroCopy")) config.enableZeroCopy = j["enableZeroCopy"];
    if (j.contains("readChunkSize")) config.readChunkSize = j["readChunkSize"];
    if (j.contains("writeChunkSize")) config.writeChunkSize = j["writeChunkSize"];
    
    // Error handling
    if (j.contains("enableErrorRecovery")) config.enableErrorRecovery = j["enableErrorRecovery"];
    if (j.contains("maxConsecutiveErrors")) config.maxConsecutiveErrors = j["maxConsecutiveErrors"];
    if (j.contains("errorResetInterval")) config.errorResetInterval = std::chrono::milliseconds(j["errorResetInterval"]);
    if (j.contains("enableCircuitBreaker")) config.enableCircuitBreaker = j["enableCircuitBreaker"];
    if (j.contains("circuitBreakerThreshold")) config.circuitBreakerThreshold = j["circuitBreakerThreshold"];
    if (j.contains("circuitBreakerTimeout")) config.circuitBreakerTimeout = std::chrono::milliseconds(j["circuitBreakerTimeout"]);
    
    return config;
}

bool StdioConfigManager::saveConfigToFile(const StdioConfig& config, const std::string& filename) const {
    try {
        ...
        spdlog::error("Failed to save stdio configuration to {}: {}", filename, e.what());
        return false;
    }
}

StdioConfig StdioConfigManager::loadConfigFromFile(const std::string& filename) const {
    try {
        ...
        spdlog::error("Failed to load stdio configuration from {}: {}", filename, e.what());
        return createDefaultConfig();
    }
}
...
json StdioConfigManager::framingModeToJson(StdioConfig::FramingMode mode) const {
    switch (mode) {
        case StdioConfig::FramingMode::NONE: return "NONE";
        case StdioConfig::FramingMode::LENGTH_PREFIX: return "LENGTH_PREFIX";
        case StdioConfig::FramingMode::DELIMITER: return "DELIMITER";
        case StdioConfig::FramingMode::JSON_LINES: return "JSON_LINES";
        case StdioConfig::FramingMode::CUSTOM: return "CUSTOM";
        default: return "DELIMITER";
    }
}

StdioConfig::FramingMode StdioConfigManager::framingModeFromJson(const json& j) const {
    std::string mode = j.get<std::string>();
    if (mode == "NONE") return StdioConfig::FramingMode::NONE;
    if (mode == "LENGTH_PREFIX") return StdioConfig::FramingMode::LENGTH_PREFIX;
    if (mode == "DELIMITER") return StdioConfig::FramingMode::DELIMITER;
    if (mode == "JSON_LINES") return StdioConfig::FramingMode::JSON_LINES;
    if (mode == "CUSTOM") return StdioConfig::FramingMode::CUSTOM;
    return StdioConfig::FramingMode::DELIMITER;
}

json StdioConfigManager::compressionTypeToJson(StdioConfig::CompressionType type) const {
    switch (type) {
        case StdioConfig::CompressionType::NONE: return "NONE";
        case StdioConfig::CompressionType::GZIP: return "GZIP";
        case StdioConfig::CompressionType::ZLIB: return "ZLIB";
        case StdioConfig::CompressionType::LZ4: return "LZ4";
        default: return "NONE";
    }
}

StdioConfig::CompressionType StdioConfigManager::compressionTypeFromJson(const json& j) const {
    std::string type = j.get<std::string>();
    if (type == "NONE") return StdioConfig::CompressionType::NONE;
    if (type == "GZIP") return StdioConfig::CompressionType::GZIP;
    if (type == "ZLIB") return StdioConfig::CompressionType::ZLIB;
    if (type == "LZ4") return StdioConfig::CompressionType::LZ4;
