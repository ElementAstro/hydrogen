...
MultiProtocolDeviceCommunicator::MultiProtocolDeviceCommunicator(const std::string& deviceId)
    : deviceId_(deviceId) {
    SPDLOG_INFO("Multi-protocol device communicator initialized for device: {}", deviceId_);
}
...
MultiProtocolDeviceCommunicator::~MultiProtocolDeviceCommunicator() {
    ...
    SPDLOG_INFO("Multi-protocol device communicator destroyed for device: {}", deviceId_);
}

bool MultiProtocolDeviceCommunicator::addProtocol(CommunicationProtocol protocol, const json& config) {
    std::lock_guard<std::mutex> lock(protocolsMutex_);
    ...
    try {
        switch (protocol) {
            case CommunicationProtocol::MQTT: {
                ...
                if (config.contains("brokerHost")) mqttConfig.brokerHost = config["brokerHost"];
                if (config.contains("brokerPort")) mqttConfig.brokerPort = config["brokerPort"];
                if (config.contains("clientId")) mqttConfig.clientId = config["clientId"];
                if (config.contains("username")) mqttConfig.username = config["username"];
                if (config.contains("password")) mqttConfig.password = config["password"];
                if (config.contains("useTls")) mqttConfig.useTls = config["useTls"];
                if (config.contains("qosLevel")) mqttConfig.qosLevel = config["qosLevel"];
                if (config.contains("topicPrefix")) mqttConfig.topicPrefix = config["topicPrefix"];
                
                auto communicator = ProtocolCommunicatorFactory::createMqttCommunicator(mqttConfig);
                
                // Set up event handlers
                communicator->setMessageHandler([this](const std::string& topic, const std::string& message) {
                    if (messageHandler_) {
            ...
            case CommunicationProtocol::GRPC: {
                ...
                if (config.contains("serverAddress")) grpcConfig.serverAddress = config["serverAddress"];
                if (config.contains("useTls")) grpcConfig.useTls = config["useTls"];
                if (config.contains("maxReceiveMessageSize")) grpcConfig.maxReceiveMessageSize = config["maxReceiveMessageSize"];
                if (config.contains("maxSendMessageSize")) grpcConfig.maxSendMessageSize = config["maxSendMessageSize"];
                if (config.contains("enableReflection")) grpcConfig.enableReflection = config["enableReflection"];
                
                auto communicator = ProtocolCommunicatorFactory::createGrpcCommunicator(grpcConfig);
            ...
            case CommunicationProtocol::ZEROMQ: {
                ZmqConfig zmqConfig;
                if (config.contains("bindAddress")) zmqConfig.bindAddress = config["bindAddress"];
                if (config.contains("connectAddress")) zmqConfig.connectAddress = config["connectAddress"];
                if (config.contains("socketType")) zmqConfig.socketType = config["socketType"];
                if (config.contains("highWaterMark")) zmqConfig.highWaterMark = config["highWaterMark"];
                if (config.contains("lingerTime")) zmqConfig.lingerTime = config["lingerTime"];
                
                ZmqCommunicator::SocketType socketType = static_cast<ZmqCommunicator::SocketType>(zmqConfig.socketType);
                auto communicator = ProtocolCommunicatorFactory::createZmqCommunicator(zmqConfig, socketType);
                
                // Set up event handlers
                communicator->setMessageHandler([this](const std::vector<std::string>& multipart) {
            ...
            case CommunicationProtocol::TCP: {
                TcpConfig tcpConfig;
                if (config.contains("serverAddress")) tcpConfig.serverAddress = config["serverAddress"];
                if (config.contains("serverPort")) tcpConfig.serverPort = config["serverPort"];
                if (config.contains("isServer")) tcpConfig.isServer = config["isServer"];
                if (config.contains("connectTimeout")) tcpConfig.connectTimeout = std::chrono::milliseconds(config["connectTimeout"]);
                if (config.contains("readTimeout")) tcpConfig.readTimeout = std::chrono::milliseconds(config["readTimeout"]);
                if (config.contains("writeTimeout")) tcpConfig.writeTimeout = std::chrono::milliseconds(config["writeTimeout"]);
                if (config.contains("bufferSize")) tcpConfig.bufferSize = config["bufferSize"];
                if (config.contains("enableKeepAlive")) tcpConfig.enableKeepAlive = config["enableKeepAlive"];
                if (config.contains("maxConnections")) tcpConfig.maxConnections = config["maxConnections"];
                if (config.contains("bindInterface")) tcpConfig.bindInterface = config["bindInterface"];

                auto communicator = ProtocolCommunicatorFactory::createTcpCommunicator(tcpConfig);

                // Set up event handlers
                communicator->setMessageHandler([this](const std::string& message, const std::string& clientId) {
                    if (messageHandler_) {
                        CommunicationMessage msg;
                        msg.messageId = generateUuid();
                        msg.deviceId = deviceId_;
                ...
                communicator->setConnectionHandler([this](bool connected, const std::string& clientId) {
                    if (connectionHandler_) {
            ...
            case CommunicationProtocol::STDIO: {
                ...
                if (config.contains("readTimeout")) stdioConfig.readTimeout = std::chrono::milliseconds(config["readTimeout"]);
                if (config.contains("writeTimeout")) stdioConfig.writeTimeout = std::chrono::milliseconds(config["writeTimeout"]);
                if (config.contains("bufferSize")) stdioConfig.bufferSize = config["bufferSize"];
                if (config.contains("lineTerminator")) stdioConfig.lineTerminator = config["lineTerminator"];
                if (config.contains("enableEcho")) stdioConfig.enableEcho = config["enableEcho"];
                if (config.contains("enableFlush")) stdioConfig.enableFlush = config["enableFlush"];
                if (config.contains("encoding")) stdioConfig.encoding = config["encoding"];

                auto communicator = ProtocolCommunicatorFactory::createStdioCommunicator(stdioConfig);
...
bool MultiProtocolDeviceCommunicator::removeProtocol(CommunicationProtocol protocol) {
    std::lock_guard<std::mutex> lock(protocolsMutex_);
...
bool MultiProtocolDeviceCommunicator::hasProtocol(CommunicationProtocol protocol) const {
    std::lock_guard<std::mutex> lock(protocolsMutex_);
...
std::vector<CommunicationProtocol> MultiProtocolDeviceCommunicator::getActiveProtocols() const {
    std::lock_guard<std::mutex> lock(protocolsMutex_);
...
bool MultiProtocolDeviceCommunicator::sendMessage(const CommunicationMessage& message, CommunicationProtocol protocol) {
    std::lock_guard<std::mutex> lock(protocolsMutex_);
    ...
    try {
        switch (protocol) {
            ...
            default:
                SPDLOG_WARN("Unsupported protocol for sending: {}", static_cast<int>(protocol));
                return false;
        }
        
    } catch (const std::exception& e) {
        SPDLOG_ERROR("Failed to send message via protocol {}: {}", static_cast<int>(protocol), e.what());
        return false;
    }
...
bool MultiProtocolDeviceCommunicator::broadcastMessage(const CommunicationMessage& message) {
    std::lock_guard<std::mutex> lock(protocolsMutex_);
    bool success = true;
...
bool MultiProtocolDeviceCommunicator::isConnected(CommunicationProtocol protocol) const {
    std::lock_guard<std::mutex> lock(protocolsMutex_);
