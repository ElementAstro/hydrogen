...
json EnhancedWebSocketError::toDetailedJson() const {
    ...
    j["connectionContext"] = {
        ...
        {"connectionStartTime", std::chrono::duration_cast<std::chrono::milliseconds>(
            connectionContext.connectionStartTime.time_since_epoch()).count()},
        {"lastActivityTime", std::chrono::duration_cast<std::chrono::milliseconds>(
            connectionContext.lastActivityTime.time_since_epoch()).count()},
...
bool EnhancedWebSocketError::shouldTriggerCircuitBreaker() const {
    ...
    return (category == WebSocketErrorCategory::CONNECTION && severity >= WebSocketErrorSeverity::HIGH) ||
           (category == WebSocketErrorCategory::NETWORK && severity >= WebSocketErrorSeverity::MEDIUM) ||
           (category == WebSocketErrorCategory::TIMEOUT && severity >= WebSocketErrorSeverity::HIGH);
}
...
std::string EnhancedWebSocketError::getErrorFingerprint() const {
    ...
    ss << static_cast<int>(category) << ":"
       ...
       << errorCode << ":"
       << connectionContext.componentName;
    return ss.str();
}

// WebSocketCircuitBreaker implementation
WebSocketCircuitBreaker::WebSocketCircuitBreaker(const std::string& connectionId)
...
bool WebSocketCircuitBreaker::canAttemptConnection() const {
    ...
    switch (state_.load()) {
        ...
        case State::OPEN: {
            ...
            auto timeSinceFailure = std::chrono::duration_cast<std::chrono::milliseconds>(
...
void WebSocketCircuitBreaker::recordSuccess() {
    ...
    if (currentState == State::HALF_OPEN) {
        ...
        if (successCount_.load() >= successThreshold_) {
            ...
            spdlog::info("WebSocketCircuitBreaker: Connection {} recovered, circuit closed", connectionId_);
        }
...
void WebSocketCircuitBreaker::recordFailure() {
    ...
    State currentState = state_.load();
    if (currentState == State::CLOSED && failureCount_.load() >= failureThreshold_) {
        state_.store(State::OPEN);
        spdlog::warn("WebSocketCircuitBreaker: Connection {} circuit opened due to {} failures", 
                     connectionId_, failureCount_.load());
    } else if (currentState == State::HALF_OPEN) {
        state_.store(State::OPEN);
        successCount_.store(0);
        spdlog::warn("WebSocketCircuitBreaker: Connection {} circuit reopened after failed recovery attempt", 
                     connectionId_);
    }
}
...
void WebSocketCircuitBreaker::reset() {
    ...
    failureCount_.store(0);
    successCount_.store(0);
    spdlog::info("WebSocketCircuitBreaker: Connection {} circuit manually reset", connectionId_);
}
...
void UnifiedWebSocketErrorHandler::registerConnection(const WebSocketConnectionContext& context) {
    ...
    spdlog::debug("UnifiedWebSocketErrorHandler: Registered connection {} for component {}", 
                  context.connectionId, context.componentName);
}

void UnifiedWebSocketErrorHandler::unregisterConnection(const std::string& connectionId) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
    connections_.erase(connectionId);
    circuitBreakers_.erase(connectionId);
    
    spdlog::debug("UnifiedWebSocketErrorHandler: Unregistered connection {}", connectionId);
}

void UnifiedWebSocketErrorHandler::updateConnectionActivity(const std::string& connectionId) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
...
void UnifiedWebSocketErrorHandler::handleEnhancedError(const EnhancedWebSocketError& error) {
    ...
    if (action != WebSocketRecoveryAction::NONE) {
        recoverySuccessful = executeRecoveryAction(error.connectionContext.connectionId, action);
...
WebSocketRecoveryAction UnifiedWebSocketErrorHandler::determineRecoveryAction(const WebSocketError& error) {
    ...
    if (recoveryStrategyCallback_) {
        try {
            ...
            spdlog::error("UnifiedWebSocketErrorHandler: Error in recovery strategy callback: {}", e.what());
        }
    }
    ...
    switch (error.category) {
        case WebSocketErrorCategory::CONNECTION:
            return (error.severity >= WebSocketErrorSeverity::MEDIUM) ? 
                   WebSocketRecoveryAction::RECONNECT : WebSocketRecoveryAction::RETRY;
        ...
        case WebSocketErrorCategory::PROTOCOL:
            return (error.severity >= WebSocketErrorSeverity::HIGH) ? 
                   WebSocketRecoveryAction::RECONNECT : WebSocketRecoveryAction::RESET;
...
bool UnifiedWebSocketErrorHandler::shouldRetry(const WebSocketError& error, int attemptCount) {
...
std::chrono::milliseconds UnifiedWebSocketErrorHandler::getRetryDelay(const WebSocketError& error, int attemptCount) {
    RetryPolicy policy = getRetryPolicy(error.component);
...
void UnifiedWebSocketErrorHandler::correlateError(const std::string& correlationId, const EnhancedWebSocketError& error) {
...
std::vector<EnhancedWebSocketError> UnifiedWebSocketErrorHandler::getCorrelatedErrors(const std::string& correlationId) const {
    std::lock_guard<std::mutex> lock(correlationMutex_);
    auto it = correlatedErrors_.find(correlationId);
    return (it != correlatedErrors_.end()) ? it->second : std::vector<EnhancedWebSocketError>{};
}

std::shared_ptr<WebSocketCircuitBreaker> UnifiedWebSocketErrorHandler::getCircuitBreaker(const std::string& connectionId) {
    std::lock_guard<std::mutex> lock(connectionsMutex_);
    auto it = circuitBreakers_.find(connectionId);
    return (it != circuitBreakers_.end()) ? it->second : nullptr;
}

void UnifiedWebSocketErrorHandler::resetCircuitBreaker(const std::string& connectionId) {
...
void UnifiedWebSocketErrorHandler::setGlobalRetryPolicy(int maxAttempts, std::chrono::milliseconds baseDelay, bool exponentialBackoff) {
...
void UnifiedWebSocketErrorHandler::setConnectionSpecificRetryPolicy(const std::string& connectionId, int maxAttempts, std::chrono::milliseconds baseDelay) {
    RetryPolicy policy;
...
UnifiedWebSocketErrorHandler::UnifiedErrorStatistics UnifiedWebSocketErrorHandler::getStatistics() const {
...
json UnifiedWebSocketErrorHandler::generateErrorReport(const std::string& connectionId) const {
            ...
            if (connectionId.empty() || connId == connectionId) {
                circuitBreakerStatus.push_back({
                    ...
                    {"lastFailureTime", std::chrono::duration_cast<std::chrono::milliseconds>(
                        breaker->getLastFailureTime().time_since_epoch()).count()}
                });
            }
        }
...
std::vector<std::string> UnifiedWebSocketErrorHandler::getTopErrorPatterns(size_t limit) const {
    std::lock_guard<std::mutex> lock(correlationMutex_);
    ...
    std::vector<std::pair<std::string, size_t>> sortedPatterns(patternCounts.begin(), patternCounts.end());
...
bool UnifiedWebSocketErrorHandler::isConnectionHealthy(const std::string& connectionId) const {
    auto circuitBreaker = const_cast<UnifiedWebSocketErrorHandler*>(this)->getCircuitBreaker(connectionId);
    if (circuitBreaker) {
...
EnhancedWebSocketError UnifiedWebSocketErrorHandler::enhanceError(const WebSocketError& error) {
...
void UnifiedWebSocketErrorHandler::updateStatistics(const EnhancedWebSocketError& error, WebSocketRecoveryAction action, bool recoverySuccessful) {
    std::lock_guard<std::mutex> lock(statisticsMutex_);
...
void UnifiedWebSocketErrorHandler::cleanupOldCorrelations() {
    ...
    for (auto it = correlatedErrors_.begin(); it != correlatedErrors_.end();) {
        ...
        for (const auto& error : it->second) {
            auto errorAge = std::chrono::duration_cast<std::chrono::milliseconds>(now - error.timestamp);
...
UnifiedWebSocketErrorHandler::RetryPolicy UnifiedWebSocketErrorHandler::getRetryPolicy(const std::string& connectionId) const {
    auto it = connectionRetryPolicies_.find(connectionId);
    return (it != connectionRetryPolicies_.end()) ? it->second : globalRetryPolicy_;
}

bool UnifiedWebSocketErrorHandler::executeRecoveryAction(const std::string& connectionId, WebSocketRecoveryAction action) {
    if (connectionRecoveryCallback_) {
        try {
            return connectionRecoveryCallback_(connectionId, action);
        } catch (const std::exception& e) {
            spdlog::error("UnifiedWebSocketErrorHandler: Error in connection recovery callback: {}", e.what());
            return false;
        }
    }
    ...
    switch (action) {
        case WebSocketRecoveryAction::RETRY:
            spdlog::info("UnifiedWebSocketErrorHandler: Retrying operation for connection {}", connectionId);
            return true; // Assume success for now
        case WebSocketRecoveryAction::RECONNECT:
            spdlog::info("UnifiedWebSocketErrorHandler: Reconnecting connection {}", connectionId);
            return true; // Assume success for now
        case WebSocketRecoveryAction::RESET:
            spdlog::info("UnifiedWebSocketErrorHandler: Resetting connection {}", connectionId);
            return true; // Assume success for now
        case WebSocketRecoveryAction::TERMINATE:
            spdlog::warn("UnifiedWebSocketErrorHandler: Terminating connection {}", connectionId);
...
void UnifiedWebSocketErrorHandler::notifyErrorEvent(const WebSocketErrorEvent& event) {
    if (errorEventCallback_) {
        try {
            errorEventCallback_(event);
        } catch (const std::exception& e) {
            spdlog::error("UnifiedWebSocketErrorHandler: Error in error event callback: {}", e.what());
        }
    }
}

// UnifiedWebSocketErrorHandlerFactory implementation
std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorHandlerFactory::createHandler() {
    return std::make_shared<UnifiedWebSocketErrorHandler>();
}

std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorHandlerFactory::createHandlerWithDefaults() {
...
std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorHandlerFactory::createClientHandler() {
...
std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorHandlerFactory::createServerHandler() {
    auto handler = createHandlerWithDefaults();
...
std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorHandlerFactory::createHighAvailabilityHandler() {
    auto handler = createHandlerWithDefaults();
...
std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorHandlerFactory::createDevelopmentHandler() {
    auto handler = createHandlerWithDefaults();
...
void UnifiedWebSocketErrorRegistry::setGlobalHandler(std::shared_ptr<UnifiedWebSocketErrorHandler> handler) {
    std::lock_guard<std::mutex> lock(handlersMutex_);
    globalHandler_ = handler;
    spdlog::info("UnifiedWebSocketErrorRegistry: Set global unified error handler");
}

std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorRegistry::getGlobalHandler() {
    std::lock_guard<std::mutex> lock(handlersMutex_);
    return globalHandler_;
}

void UnifiedWebSocketErrorRegistry::registerComponentHandler(const std::string& component, std::shared_ptr<UnifiedWebSocketErrorHandler> handler) {
    std::lock_guard<std::mutex> lock(handlersMutex_);
    componentHandlers_[component] = handler;
    spdlog::info("UnifiedWebSocketErrorRegistry: Registered unified error handler for component: {}", component);
}

void UnifiedWebSocketErrorRegistry::unregisterComponentHandler(const std::string& component) {
    std::lock_guard<std::mutex> lock(handlersMutex_);
    auto it = componentHandlers_.find(component);
    if (it != componentHandlers_.end()) {
        componentHandlers_.erase(it);
        spdlog::info("UnifiedWebSocketErrorRegistry: Unregistered unified error handler for component: {}", component);
    }
}

std::shared_ptr<UnifiedWebSocketErrorHandler> UnifiedWebSocketErrorRegistry::getComponentHandler(const std::string& component) {
    std::lock_guard<std::mutex> lock(handlersMutex_);
    auto it = componentHandlers_.find(component);
    return (it != componentHandlers_.end()) ? it->second : globalHandler_;
}

void UnifiedWebSocketErrorRegistry::handleError(const WebSocketError& error, const std::string& component) {
    auto handler = getComponentHandler(component.empty() ? error.component : component);
    if (handler) {
        handler->handleError(error);
    } else {
        spdlog::error("UnifiedWebSocketErrorRegistry: No handler available for error in component {}: {}",
                     component.empty() ? error.component : component, error.toString());
    }
}

void UnifiedWebSocketErrorRegistry::handleEnhancedError(const EnhancedWebSocketError& error, const std::string& component) {
    auto handler = getComponentHandler(component.empty() ? error.component : component);
    if (handler) {
        handler->handleEnhancedError(error);
    } else {
        spdlog::error("UnifiedWebSocketErrorRegistry: No handler available for enhanced error in component {}: {}",
                     component.empty() ? error.component : component, error.toString());
    }
}

UnifiedWebSocketErrorHandler::UnifiedErrorStatistics UnifiedWebSocketErrorRegistry::getGlobalStatistics() const {
    std::lock_guard<std::mutex> lock(handlersMutex_);
