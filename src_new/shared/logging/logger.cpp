...
#else
    // Fallback when spdlog is not available
    std::cout << "Logger initialized (fallback mode) - file: " << filename << std::endl;
...
#else
    // Fallback when spdlog is not available
    std::cout << "[WARN]" << (component.empty() ? "" : "[" + component + "]") << " " << message << std::endl;
#endif
}
