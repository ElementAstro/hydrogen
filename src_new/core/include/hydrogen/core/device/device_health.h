...
public:
  ...
  virtual void recordCommand(const std::string &deviceId,
                             const std::string &command, double responseTimeMs,
                             bool success) = 0;
   ...
   */
  virtual void recordConnection(const std::string &deviceId, bool success,
   ...
   */
  virtual DeviceHealthStatus
  getHealthStatus(const std::string &deviceId) const = 0;
   ...
   */
  virtual DeviceHealthReport
  getHealthReport(const std::string &deviceId) const = 0;
   ...
   */
  virtual void setHealthThreshold(DeviceHealthStatus status,
                                  double threshold) = 0;
...
public:
  ...
  void recordCommand(const std::string &deviceId, const std::string &command,
                     double responseTimeMs, bool success) override;

  void recordConnection(const std::string &deviceId, bool success,
                        double connectionTimeMs) override;

  void recordError(const std::string &deviceId, const std::string &errorCode,
                   const std::string &severity) override;

  void updateResourceUsage(const std::string &deviceId, double memoryUsageMB,
                           double cpuUsagePercent) override;

  DeviceHealthStatus
  getHealthStatus(const std::string &deviceId) const override;
  DeviceHealthReport
  getHealthReport(const std::string &deviceId) const override;
...
private:
  ...
  generateHealthIssues(const DeviceMetrics &metrics) const;
  std::vector<std::string>
  generateRecommendations(const DeviceMetrics &metrics) const;
};
