...
public:
  ...
  // Event handlers
  void setMessageHandler(StreamMessageHandler handler) {
    messageHandler_ = handler;
  }
  void setErrorHandler(StreamErrorHandler handler) { errorHandler_ = handler; }
  void setStatusHandler(StreamStatusHandler handler) {
...
private:
  ...
  std::unique_ptr<grpc::ClientReaderWriter<astrocomm::proto::Message,
                                           astrocomm::proto::Message>>
  ...
  convertFromProtoMessage(const json &protoMessage) const;
...
public:
  using StreamRequestHandler = std::function<void(
      const Message &, grpc::ServerWriter<astrocomm::proto::Message> *)>;
  using ClientStreamHandler = std::function<CommunicationResponse(
      grpc::ServerReader<astrocomm::proto::Message> *)>;
  using BidirectionalStreamHandler = std::function<void(
      grpc::ServerReaderWriter<astrocomm::proto::Message,
                               astrocomm::proto::Message> *)>;
  ...
  // Stream handlers
  void setServerStreamHandler(StreamRequestHandler handler) {
    serverStreamHandler_ = handler;
  }
  void setClientStreamHandler(ClientStreamHandler handler) {
    clientStreamHandler_ = handler;
  }
  void setBidirectionalStreamHandler(BidirectionalStreamHandler handler) {
    bidirectionalStreamHandler_ = handler;
  }
...
public:
  static std::unique_ptr<GrpcStreamingClient>
  createStreamingClient(const GrpcConfig &config);
  static std::unique_ptr<GrpcStreamingServer>
  createStreamingServer(const GrpcConfig &config);
