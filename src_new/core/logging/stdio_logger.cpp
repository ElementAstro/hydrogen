...
StdioLogger::LoggerConfig StdioLogger::getConfig() const { return config_; }

void StdioLogger::trace(const std::string &message,
  ...
  std::cout << "[TRACE] " << formatLogMessage(message, clientId) << std::endl;
#endif
}

void StdioLogger::debug(const std::string &message,
                        const std::string &clientId) {
#ifdef HYDROGEN_HAS_SPDLOG
...
#else
  std::cout << "[DEBUG] " << formatLogMessage(message, clientId) << std::endl;
#endif
}

void StdioLogger::info(const std::string &message,
                       const std::string &clientId) {
#ifdef HYDROGEN_HAS_SPDLOG
...
#else
  std::cout << "[INFO] " << formatLogMessage(message, clientId) << std::endl;
#endif
}

void StdioLogger::warn(const std::string &message,
                       const std::string &clientId) {
#ifdef HYDROGEN_HAS_SPDLOG
...
#else
  std::cout << "[WARN] " << formatLogMessage(message, clientId) << std::endl;
#endif
}

void StdioLogger::error(const std::string &message,
                        const std::string &clientId) {
#ifdef HYDROGEN_HAS_SPDLOG
  if (logger_ && logger_->level() <= spdlog::level::err) {
    logger_->error(formatLogMessage(message, clientId));
  }
#else
  std::cerr << "[ERROR] " << formatLogMessage(message, clientId) << std::endl;
#endif
  updateErrorStats(message);
}

void StdioLogger::critical(const std::string &message,
                           const std::string &clientId) {
#ifdef HYDROGEN_HAS_SPDLOG
...
#else
  std::cerr << "[CRITICAL] " << formatLogMessage(message, clientId) << std::endl;
#endif
  updateErrorStats(message);
}
...
void StdioLogger::traceMessage(const MessageTrace &trace) {
  if (!config_.enableMessageTracing ||
      !shouldTrace(trace.clientId, trace.messageType)) {
    return;
  }
...
#else
  // Fallback trace logging to console
  std::ostringstream oss;
  oss << "[TRACE] [" << trace.direction << "] "
      << "Client: " << trace.clientId << ", "
      << "Type: " << trace.messageType << ", "
      << "Size: " << formatBytes(trace.messageSize) << ", "
      << "Time: " << formatDuration(trace.processingTime) << ", "
...
void StdioLogger::traceIncomingMessage(const std::string &messageId,
                                       const std::string &clientId,
                                       const std::string &messageType,
                                       const json &content, size_t size) {
  MessageTrace trace;
...
void StdioLogger::traceOutgoingMessage(const std::string &messageId,
                                       const std::string &clientId,
                                       const std::string &messageType,
                                       const json &content, size_t size) {
  MessageTrace trace;
...
void StdioLogger::traceMessageProcessing(
    const std::string &messageId, std::chrono::microseconds processingTime,
...
void StdioLogger::logClientConnect(
    const std::string &clientId,
...
void StdioLogger::logClientDisconnect(const std::string &clientId,
...
void StdioLogger::logConnectionError(const std::string &clientId,
...
void StdioLogger::logProtocolEvent(const std::string &event,
                                   const std::string &details,
                                   const std::string &clientId) {
  debug("Protocol event: " + event + " - " + details, clientId);
}

void StdioLogger::logProtocolError(const std::string &error,
                                   const std::string &clientId) {
...
void StdioLogger::logValidationError(const std::string &messageId,
                                     const std::string &error,
                                     const std::string &clientId) {
  this->error("Validation error for message " + messageId + ": " + error,
              clientId);
...
void StdioLogger::recordMessage(bool success, size_t bytes,
  ...
  while (
      timeUs < currentMin &&
      !metrics_.minProcessingTime.compare_exchange_weak(currentMin, timeUs)) {
  ...
  while (
      timeUs > currentMax &&
      !metrics_.maxProcessingTime.compare_exchange_weak(currentMax, timeUs)) {
    // Retry if another thread updated the value
  }
}
...
void StdioLogger::resetMetrics() {
  // Reset atomic values individually
  metrics_.totalMessages = 0;
  metrics_.successfulMessages = 0;
  metrics_.failedMessages = 0;
  metrics_.totalBytes = 0;
  metrics_.totalProcessingTime = 0;
  metrics_.minProcessingTime = UINT64_MAX;
  metrics_.maxProcessingTime = 0;
  metrics_.totalConnections = 0;
  metrics_.activeConnections = 0;
  metrics_.connectionErrors = 0;
  metrics_.protocolErrors = 0;
  metrics_.timeoutErrors = 0;
...
void StdioLogger::setDebugFilter(const std::vector<std::string> &clientIds,
...
void StdioLogger::dumpDebugInfo(const std::string &filename) const {
  ...
  if (filename.empty()) {
    const_cast<StdioLogger*>(this)->info("Debug info: " + debugInfo.dump(2));
  } else {
    std::ofstream file(filename);
    file << debugInfo.dump(2);
    file.close();
    const_cast<StdioLogger*>(this)->info("Debug info saved to: " + filename);
  }
}
...
StdioLogger::getMessageHistory(const std::string &clientId) const {
  std::lock_guard<std::mutex> lock(historyMutex_);
...
json StdioLogger::generateReport(
...
std::vector<std::string> StdioLogger::getTopErrors(size_t count) const {
  ...
  std::vector<std::pair<std::string, uint64_t>> errorPairs(errorCounts_.begin(),
                                                           errorCounts_.end());
  ...
  for (size_t i = 0; i < std::min(count, errorPairs.size()); ++i) {
    topErrors.push_back(errorPairs[i].first + " (" +
                        std::to_string(errorPairs[i].second) + ")");
  }
...
std::vector<std::string> StdioLogger::getMostActiveClients(size_t count) const {
  ...
  std::vector<std::pair<std::string, uint64_t>> clientPairs(
      clientActivity_.begin(), clientActivity_.end());
  ...
  for (size_t i = 0; i < std::min(count, clientPairs.size()); ++i) {
    activeClients.push_back(clientPairs[i].first + " (" +
                            std::to_string(clientPairs[i].second) + ")");
  }
  ...
  try {
    ...
    if (config_.enableAsyncLogging) {
      ...
      logger_ = std::make_shared<spdlog::async_logger>(
          "stdio_logger", sinks.begin(), sinks.end(), spdlog::thread_pool(),
          spdlog::async_overflow_policy::block);
    } else {
      logger_ = std::make_shared<spdlog::logger>("stdio_logger", sinks.begin(),
                                                 sinks.end());
    }
    ...
    case StdioLogLevel::TRACE:
      level = spdlog::level::trace;
      break;
    case StdioLogLevel::DEBUG:
      level = spdlog::level::debug;
      break;
    case StdioLogLevel::INFO:
      level = spdlog::level::info;
      break;
    case StdioLogLevel::WARN:
      level = spdlog::level::warn;
      break;
    case StdioLogLevel::ERR:
      level = spdlog::level::err;
      break;
    case StdioLogLevel::CRITICAL:
      level = spdlog::level::critical;
      break;
    case StdioLogLevel::OFF:
    ...
    if (config_.enableMessageTracing) {
      ...
      traceLogger_ =
          std::make_shared<spdlog::logger>("stdio_trace", trace_sink);
...
#else
  // No spdlog available - use console fallback
  // Logger initialization is not needed for console fallback
#endif
}

bool StdioLogger::shouldTrace(const std::string &clientId,
                              const std::string &messageType) const {
  // Check client filter
  if (!config_.trackedClients.empty()) {
    if (std::find(config_.trackedClients.begin(), config_.trackedClients.end(),
                  clientId) == config_.trackedClients.end()) {
      return false;
    }
  }
  ...
  if (!config_.trackedMessageTypes.empty()) {
    if (std::find(config_.trackedMessageTypes.begin(),
                  config_.trackedMessageTypes.end(),
                  messageType) == config_.trackedMessageTypes.end()) {
      return false;
    }
  }
...
std::string StdioLogger::formatLogMessage(const std::string &message,
...
json StdioLogger::messageTraceToJson(const MessageTrace &trace) const {
  ...
  j["timestamp"] = std::chrono::duration_cast<std::chrono::milliseconds>(
                       trace.timestamp.time_since_epoch())
                       .count();
...
MessageTracer::MessageTracer(StdioLogger &logger, const std::string &messageId,
    ...
    : logger_(logger), messageId_(messageId), clientId_(clientId),
      operation_(operation), startTime_(std::chrono::system_clock::now()) {}
...
void MessageTracer::setSuccess(bool success) { success_ = success; }
...
void MessageTracer::addMetadata(const std::string &key,
