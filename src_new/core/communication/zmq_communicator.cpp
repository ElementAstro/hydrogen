...
public:
  ZmqCommunicatorImpl(const ZmqConfig &config, SocketType socketType)
    ...
    SPDLOG_INFO(
        "ZeroMQ Communicator initialized with socket type {} for address {}",
        static_cast<int>(socketType), config_.bindAddress);
  }

  ~ZmqCommunicatorImpl() { disconnect(); }
  ...
  bool bind(const std::string &address) override {
    try {
      ...
      // socket_ = std::make_unique<zmq::socket_t>(*context_,
      // static_cast<int>(socketType_));
      //
      // // Set socket options
      ...
      if (socketType_ == SocketType::REP || socketType_ == SocketType::SUB ||
          socketType_ == SocketType::PULL) {
  ...
  bool connect(const std::string &address) override {
    try {
      ...
      // Start message processing thread for certain socket types
      if (socketType_ == SocketType::REQ || socketType_ == SocketType::SUB ||
          socketType_ == SocketType::PULL) {
        messageThread_ = std::thread(&ZmqCommunicatorImpl::messageLoop, this);
      }
  ...
  bool send(const std::string &message, bool nonBlocking) override {
    ...
    try {
      ...
      //
      // zmq::send_flags flags = nonBlocking ? zmq::send_flags::dontwait :
      // zmq::send_flags::none; auto result = socket_->send(zmqMessage, flags);
      //
  ...
  bool send(const std::vector<std::string> &multipart,
    ...
    try {
      SPDLOG_DEBUG("Sending ZeroMQ multipart message with {} parts",
                   multipart.size());
  ...
  bool receive(std::string &message, bool nonBlocking) override {
    ...
    try {
      ...
      // zmq::recv_flags flags = nonBlocking ? zmq::recv_flags::dontwait :
      // zmq::recv_flags::none;
      //
      // auto result = socket_->recv(zmqMessage, flags);
      // if (result.has_value()) {
      //     message = std::string(static_cast<char*>(zmqMessage.data()),
      //     zmqMessage.size()); return true;
      // }
  ...
  bool receive(std::vector<std::string> &multipart, bool nonBlocking) override {
    ...
    try {
      ...
      //     }
      //
      //     multipart.emplace_back(static_cast<char*>(part.data()),
      //     part.size());
      // } while (part.more());
  ...
  bool unsubscribe(const std::string &filter) override {
    ...
    try {
      ...
      SPDLOG_ERROR("Failed to unsubscribe from filter '{}': {}", filter,
                   e.what());
      return false;
    }
  }
...
std::unique_ptr<ZmqCommunicator>
ProtocolCommunicatorFactory::createZmqCommunicator(
    const ZmqConfig &config, ZmqCommunicator::SocketType socketType) {
